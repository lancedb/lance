...
class LanceDataset(pa.dataset.Dataset):
    ...
    def create_index(
        ...
        **Experimental API**
        ...
        fragment_ids : List[int], optional
            ...
            to the dataset. The index can be committed later using merge_index_metadata(index_uuid, "VECTOR", column=..., index_name=...).
        fragment_uuid : str, optional
        ...
        * `Product quantization for nearest neighbor search
        ...
        if index_type.startswith("IVF"):
            ...
            if (ivf_centroids is None) and (pq_codebook is not None):
                warnings.warn("pq_codebook is ignored; global codebook will be trained", UserWarning)


            if ivf_centroids is not None:
                if _check_for_numpy(ivf_centroids) and isinstance(ivf_centroids, np.ndarray):
                    if len(ivf_centroids.shape) != 2 or ivf_centroids.shape[0] != num_partitions:
                        raise ValueError(f"Ivf centroids must be 2D array: (clusters, dim), got {ivf_centroids.shape}")
                    if ivf_centroids.dtype not in [np.float16, np.float32, np.float64]:
                        raise TypeError(f"IVF centroids must be floating number, got {ivf_centroids.dtype}")
        ...
        if "PQ" in index_type:
            ...
            if pq_codebook is not None:
                ...
                if _check_for_numpy(pq_codebook) and isinstance(pq_codebook, np.ndarray):
                    if (
        ...
        # Add fragment_ids and fragment_uuid to kwargs if provided for distributed indexing
        if fragment_ids is not None:
            kwargs["fragment_ids"] = fragment_ids
        if fragment_uuid is not None:
            kwargs["fragment_uuid"] = fragment_uuid
    ...
    def merge_index_metadata(
        ...
        column : str, optional
            ...
            If omitted, the system will pick a reasonable column candidate based on index_type:
            - VECTOR: first vector column (FixedSizeList or 1D FixedShapeTensor)
            - BTREE: first scalar column (int/float/bool/string/fixed-size-binary/temporal)
        ...
        Examples
        --------
        # Merge distributed vector index and commit:
        >>> dataset.merge_index_metadata("shared-uuid-123", "VECTOR", column="embeddings", index_name="embeddings_idx")

        # Merge inverted index and commit:
        >>> dataset.merge_index_metadata("shared-uuid-fts", "INVERTED", column="doc", index_name="doc_fts")
        """
        # Normalize type
        t = index_type.upper()
        ...
        if t not in valid:
            raise NotImplementedError(
                f'Only "VECTOR", "BTREE" or "INVERTED" are supported, received {index_type}'
            )
        ...
        if column is None:
            ...
            if column is None:
                raise ValueError(
                    "Unable to infer a suitable column for the given index type; please provide 'column'."
                )
