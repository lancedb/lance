        ...
        if index_type == "IVF_PQ":
            ...
            if ivf_centroids is not None:
                ...
                if isinstance(ivf_centroids, np.ndarray):
                    ...
                    elif ivf_centroids.dtype == np.float16:
                        values = pa.array(ivf_centroids.reshape(-1), type=pa.float16())
                    else:
                        raise TypeError(
                            (
                                "IVF centroids must be float32 or float16,"
                                " got {ivf_centroids.dtype}"
                            )
                        )
