...
def test_merge_insert(tmp_path: Path):
    ...
    merge_dict = dataset.merge_insert("a").when_not_matched_insert_all().execute(new_table)
    table = dataset.to_table()
    assert table.num_rows == 1300
    assert table.filter(is_new).num_rows == 300
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (300, 0, 0)
    ...
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (0, 700, 0)

    dataset = lance.dataset(tmp_path / "dataset", version=version)
    dataset.restore()
    merge_dict = dataset.merge_insert(
        "a"
    ).when_not_matched_insert_all().when_matched_update_all().execute(new_table)
    table = dataset.to_table()
    assert table.num_rows == 1300
    assert table.filter(is_new).num_rows == 1000
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (300, 700, 0)

    dataset = lance.dataset(tmp_path / "dataset", version=version)
    dataset.restore()
    merge_dict = dataset.merge_insert("a").when_not_matched_insert_all().when_matched_update_all(
        "target.c == source.c"
    ).execute(new_table)
    table = dataset.to_table()
    assert table.num_rows == 1300
    assert table.filter(is_new).num_rows == 650
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (300, 350, 0)

    dataset = lance.dataset(tmp_path / "dataset", version=version)
    dataset.restore()
    merge_dict = dataset.merge_insert("a").when_not_matched_by_source_delete().execute(new_table)
    table = dataset.to_table()
    assert table.num_rows == 700
    assert table.filter(is_new).num_rows == 0
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (0, 0, 300)

    dataset = lance.dataset(tmp_path / "dataset", version=version)
    dataset.restore()
    merge_dict = dataset.merge_insert("a").when_not_matched_by_source_delete(
        "a < 100"
    ...
    assert table.filter(is_new).num_rows == 300
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (300, 0, 100)
    ...
    with pytest.raises(ValueError):
        ...
        assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (None, None, None)
...
def test_merge_insert_conditional_upsert_example(tmp_path: Path):
    ...
    merge_dict = dataset.merge_insert("id").when_matched_update_all(
    ...
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (0, 2, 0)
    ...
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (0, 0, 0)
...
def test_merge_insert_source_is_dataset(tmp_path: Path):
    ...
    merge_dict = dataset.merge_insert("a").when_not_matched_insert_all().execute(new_dataset)
    table = dataset.to_table()
    assert table.num_rows == 1300
    assert table.filter(is_new).num_rows == 300
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (300, 0, 0)
    ...
    merge_dict = dataset.merge_insert("a").when_not_matched_insert_all().execute(
...
def test_merge_insert_multiple_keys(tmp_path: Path):
    ...
    merge_dict = dataset.merge_insert(["a", "c"]).when_matched_update_all().execute(new_table)
    table = dataset.to_table()
    assert table.num_rows == 1000
    assert table.filter(is_new).num_rows == 350
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (0, 350, 0)
...
def test_merge_insert_vector_column(tmp_path: Path):
    ...
    assert (merge_dict["num_inserted_rows"], merge_dict["num_updated_rows"], merge_dict["num_deleted_rows"]) == (1, 1, 0)
