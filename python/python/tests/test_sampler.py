...
import pytest
from lance.sampler import ShardedBatchSampler, maybe_sample
...
def test_data_stream_without_filter(sample_dataset):
    ...
    sampler = ShardedBatchSampler(0, 4)
...
def test_edge_cases():
    ...
    # Initialize with non-divisible data size
    sampler = ShardedBatchSampler(
        rank=3, world_size=4, total_num_rows=999, batch_size=250
    )

    batches = list(sampler)
    assert len(batches) == 1, "Should handle partial batch"
    assert batches[0] == list(range(750, 999)), "Last rank should get 750-998"
