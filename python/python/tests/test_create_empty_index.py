...
def test_create_empty_scalar_index(tmp_path):
    ...
    stats = dataset.stats.index_stats(indices[0]["name"])
    # Index might be populated or empty - just verify it exists and has reasonable values
...
def test_create_empty_vector_index(tmp_path):
    ...
    dim = 128
    vectors = pa.array(np.random.randn(100, dim).tolist(), 
    ...
    # TODO: This should raise an error when proper empty vector index implementation is added
    try:
        dataset.create_index("vector", "IVF_PQ", num_partitions=10, 
                           num_sub_vectors=8, train=False)
