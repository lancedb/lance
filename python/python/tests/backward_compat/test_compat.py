...
# Backward compatibility tests for older versions of Lance
#
# This file will be run on current version of Lance to test that the
# older versions of Lance can read the test data generated by datagen.py.
...
from .util import build_basic_types, build_large, get_path, old_version


@pytest.mark.backward
@pytest.mark.skipif(
    old_version() is not None and old_version() < Version("0.36.0"),  # at least 0.36.0
...
@pytest.mark.backward
@pytest.mark.skipif(
    old_version() is not None and old_version() < Version("0.20.0"),
...
@pytest.mark.backward
@pytest.mark.skipif(
    old_version() is not None and old_version() < Version("0.29.1.beta2"),
...
@pytest.mark.backward
@pytest.mark.skipif(
    old_version() is not None and old_version() < Version("0.36.0"),
