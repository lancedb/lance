...
def test_index_default_codebook(tmp_path):
    """Ensure default global codebook (no user-supplied pq_codebook) builds and validates."""
    tbl = create_table(nvec=1024, ndim=128)
    dataset = lance.write_dataset(tbl, tmp_path)

    # Default build without supplying pq_codebook; internal training uses global unified codebook
    dataset = dataset.create_index(
        "vector",
        index_type="IVF_PQ",
        num_partitions=1,
...
# =============================================================================
# Distributed vector index consistency helper (merged from test_vector_distributed_consistency)
# =============================================================================
...
def compare_vector_results(
    ...
    *,
    ...
    - If _distance columns exist in both results, compare per-ID distances within tolerance

    Raises AssertionError with clear, English diagnostics on mismatch.
    """
    # Row count check
    assert (
        single_results.num_rows == distributed_results.num_rows
    ), (
        f"Row count mismatch{f' for query #{query_id}' if query_id is not None else ''}: "
        f"single={single_results.num_rows}, distributed={distributed_results.num_rows}"
    )
    ...
    if "id" not in single_results.column_names or "id" not in distributed_results.column_names:
    ...
    if "_distance" in single_results.column_names and "_distance" in distributed_results.column_names:
        ...
        for sid in single_set:
            ...
            assert diff <= tolerance, (
                f"Distance mismatch{f' for query #{query_id}' if query_id is not None else ''} "
                f"on id={sid}: single={s_val}, distributed={d_val}, tolerance={tolerance}"
            )
...
def assert_distributed_vector_consistency(
    ...
    *,
    ...
    """Compare single vs distributed ANN TopK by similarity metrics (Recall/Jaccard) or strict match.
    ...
    tmp_path : Path-like, optional
        If provided, datasets will be written to tmp_path / single and tmp_path / distributed.
        If not provided, writes to a temporary local directory.
    similarity_metric : str, default "strict"
        One of {"strict", "recall", "jaccard"}. "strict" enforces identical TopK ID sets.
    ...
    dist_ds = build_distributed_vector_index(
        ...
        world=world,
        **{k: v for k, v in index_params.items() if k not in {"num_partitions", "num_sub_vectors"}},
    )
    ...
    for i, q in enumerate(queries or []):
        ...
        single_res = single_ds.to_table(nearest=nearest, columns=["id", "_distance"])  # payload minimized
        dist_res = dist_ds.to_table(nearest=nearest, columns=["id", "_distance"])    # same projection

        if similarity_metric == "strict":
            compare_vector_results(single_res, dist_res, tolerance=tolerance, query_id=i)
            continue

        # Compute similarity metrics against exact search (use_index=False) as ground truth
        gt_nearest = {"column": column, "q": q, "k": topk, "use_index": False}
        gt_res = single_ds.to_table(nearest=gt_nearest, columns=["id", "_distance"])  # precise TopK

        ground_ids = gt_res["id"].to_pylist()
        dist_ids = dist_res["id"].to_pylist()
        recall, jaccard, inter_cnt, union_cnt = _compute_similarity_metrics(ground_ids, dist_ids)
        ...
        if "_distance" in gt_res.column_names and "_distance" in dist_res.column_names:
            s_map = {int(i): float(d) for i, d in zip(ground_ids, gt_res["_distance"].to_pylist())}
            d_map = {int(i): float(d) for i, d in zip(dist_ids, dist_res["_distance"].to_pylist())}
            for sid in set(ground_ids) & set(dist_ids):
                diff = abs(s_map[sid] - d_map[sid])
                assert diff <= tolerance, (
                    f"Distance mismatch vs exact for query #{i} on id={sid}: "
                    f"exact={s_map[sid]}, distributed={d_map[sid]}, tolerance={tolerance}"
                )
...
def test_distributed_api_basic_success(tmp_path):
    ...
    fragment_ids = [frags[0].fragment_id] + ([frags[1].fragment_id] if len(frags) > 1 else [])
    ds.create_index(
        column="vector",
        index_type="IVF_PQ",
...
@pytest.mark.parametrize(
    ...
    [
        (
            "scattered_fragments",
            lambda fs: [fs[0].fragment_id, fs[2].fragment_id] if len(fs) >= 3 else [fs[0].fragment_id],
        ),
...
def test_fragment_allocations_divisibility_error(tmp_path, case_name, selector):
    ...
    shared_uuid = str(_uuid.uuid4())
    with pytest.raises(ValueError, match=r"dimension .* must be divisible by num_sub_vectors"):
        ds.create_index(
            column="vector",
            index_type="IVF_PQ",
            fragment_ids=fragment_ids,
...
def test_distributed_workflow_merge_and_search(tmp_path):
    """End-to-end: build IVF_PQ on two groups, merge, and verify search returns results."""
    ds = _make_sample_dataset(tmp_path, n_rows=2000)
    frags = ds.get_fragments()
    if len(frags) < 2:
