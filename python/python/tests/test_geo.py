...
geoarrow = pytest.importorskip("geoarrow.rust.core")
...
def test_geo_types(tmp_path: Path):
    ...
    polygons_2d = polygons([np.random.rand(12), np.random.rand(12)], ring_offsets=ring_offsets, geom_offsets=geom_offsets)
    ...
    table = pa.Table.from_arrays([points_2d, linestrings_2d, polygons_2d], schema=schema)
    ...
    assert read_table.schema.field(0).metadata[b'ARROW:extension:name'] == b'geoarrow.point'
    assert read_table.schema.field(1).metadata[b'ARROW:extension:name'] == b'geoarrow.linestring'
    assert read_table.schema.field(2).metadata[b'ARROW:extension:name'] == b'geoarrow.polygon'
...
def test_geo_udf_distance(tmp_path: Path):
    ...
    linestrings_2d = linestrings([np.array([3.0, 4.0]), np.array([5.0, 0.0])], line_offsets)
    ...
    batches = ds.sql("SELECT St_Distance(point, linestring) as dist FROM dataset").build().to_batch_records()
    assert len(batches) == 1
    result = batches[0].to_pydict()
    assert result["dist"]
    assert np.allclose(np.array(result["dist"]), np.array([2.5495097567963922]), atol=1e-8)
