...
use crate::{
    dataset::Dataset, error::PythonErrorExt, file::object_store_from_uri_or_path_no_options, global_executor
};
...
fn get_ivf_model(py: Python<'_>, dataset: &Dataset, index_name: &str) -> PyResult<Py<PyIvfModel>> {
    let ivf_model = global_executor().block_on(Some(py), do_get_ivf_model(dataset, index_name))??;
...
fn train_ivf_model(
    ...
    let centroids = global_executor().block_on(
        Some(py),
        do_train_ivf_model(
            dataset,
            column,
...
fn train_pq_model(
    ...
    let codebook = global_executor().block_on(
        Some(py),
        do_train_pq_model(
            dataset,
            column,
...
pub fn shuffle_transformed_vectors(
    ...
    let result = global_executor().block_on(
        None,
