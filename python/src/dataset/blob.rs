...
use crate::{error::PythonErrorExt, global_executor};
...
impl LanceBlobFile {
    pub fn close(&self, py: Python<'_>) -> PyResult<()> {
        ...
        global_executor().block_on(Some(py), inner.close())?.infer_error()
    }

    pub fn is_closed(&self, py: Python<'_>) -> PyResult<bool> {
        let inner = self.inner.clone();
        global_executor().block_on(Some(py), inner.is_closed())
    }

    pub fn seek(&self, py: Python<'_>, position: u64) -> PyResult<()> {
        let inner = self.inner.clone();
        global_executor().block_on(Some(py), inner.seek(position))?.infer_error()
    }

    pub fn tell(&self, py: Python<'_>) -> PyResult<u64> {
        let inner = self.inner.clone();
        global_executor().block_on(Some(py), inner.tell())?.infer_error()
    }
    ...
    pub fn readall<'a>(&'a self, py: Python<'a>) -> PyResult<Bound<'a, PyBytes>> {
        let inner = self.inner.clone();
        let data = global_executor().block_on(Some(py), inner.read())?.infer_error()?;
