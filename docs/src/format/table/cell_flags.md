...
The transaction protobuf carries one typed `cell_flag_transaction` sidecar. It records the dataset incarnation, registry changes, existing-row address changes, exact state for newly written fragments, field-ID transfers used by schema casts, stable flag IDs read to derive persisted values or mutation decisions, the operation's complete existing-row rewrite or deletion set when rebasing needs it, and a fixed-size commitment to the public operation fields. Read dependencies are valid on update, delete, and merge operations; they do not authorize flag changes. The sidecar is internal replay and conflict-detection material, is hidden by language bindings, and does not introduce arbitrary per-row keys or policy callbacks. The source-compatible native Rust carrier uses a reserved in-memory property entry; protobuf and language-binding conversion extracts it into the typed sidecar and excludes it from application transaction properties. Callers that replace application properties or regenerate a UUID on a returned native transaction must use the carrier-preserving `Transaction` methods rather than reconstructing opaque internal entries.
...
Concurrent registry edits conflict with every Cell Flag read or write. A transaction that reads a flag conflicts with a concurrent write to the same stable flag ID; this dependency is intentionally tracked at whole-flag granularity. Explicit row changes still use the existing exact-row mutation conflict machinery. A physical column merge that neither reads nor writes Cell Flags remains compatible with a disjoint flag-only update. Atomicity does not establish application-level freshness beyond these recorded dependencies; systems that need broader freshness must validate their own source revision or read set.

Dataset-level operations carry these dependencies automatically. Fragment-level staged `add_columns` and `delete` operations reject `cell_flag` expressions because their returned fragment metadata cannot carry the typed read dependency; callers must use the corresponding dataset-level operation.
