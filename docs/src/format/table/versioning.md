...
| 512      | `FLAG_STABLE_FIELD_IDS`         | No              | Yes             | After explicit migration, the manifest carries a persistent field-ID high-water mark that writers must use when allocating field IDs. Operators must retire older writers before activation. See [Field IDs](schema.md#field-ids). |
