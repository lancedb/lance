...
| 256      | `FLAG_MIXED_DATA_FILE_VERSIONS` | Yes             | Yes             | Reserved for datasets that reference recognized V2 data files with different exact versions. Implementations must reject this bit until they support the per-file exact-version contract. |
| 512      | `FLAG_JSON_EXACT_NUMBERS`       | Yes             | Yes             | JSON columns may contain exact decimal JSONB encodings. Readers without arbitrary-precision JSONB support would silently round these values through `float64`. |
