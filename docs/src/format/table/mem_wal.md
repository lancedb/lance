...
Two rules bound what a commit may record. It never credits more than its own `compacted_sstables`, so a read version since rolled back cannot retire SSTables no live commit copied in. And it never lowers a position an index already held, provided that index is unchanged by this commit: an index is compared by its full segment metadata, not by segment UUID, because an operation can prune a segment's fragment bitmap in place while keeping its UUID. An index this commit changes keeps no position it cannot re-earn.

Because the position is derived rather than transmitted, it cannot go stale between inspection and commit, it survives rebase (`read_version` is fixed for a transaction's life), and any operation can earn it — an ordinary index build that happens to cover the table records catch-up as a side effect, with no dedicated repair.
