# 使用 HNSW 索引数据集（Hierarchical Navigable Small World）

HNSW 是一种基于图的算法，用于高维空间中的近似最近邻搜索（Approximate Nearest Neighbor Search）。在本示例中，我们将演示如何对 Lance 数据集构建 HNSW 向量索引。

本示例将展示如何：

1. 生成指定维度的合成测试数据
2. 使用 Lance API 构建用于高效向量搜索的层次化图结构
3. 使用不同参数执行向量搜索，并使用 L2 距离搜索计算真实值（Ground Truth）
