# 在 Rust 中使用 Lance 创建用于 LLM 训练的文本数据集

在本示例中，我们将演示如何在 Rust 中实现 Python 示例中展示的 LLM 数据集创建功能。

!!! note
    HuggingFace 的 Python API 支持以流式模式加载数据，且内置了随机打乱功能。Rust API 缺少这些特性，因此数据需要手动下载并在每个批次内进行打乱。

本示例将展示如何：

1. 从 HuggingFace 分批下载和处理文本数据集
2. 使用自定义 RecordBatchReader 对文本数据进行分词
3. 使用 Lance API 将其保存为 Lance 数据集

Rust 中的实现细节将遵循与 Python 版本类似的概念，但使用 Rust 特有的 API 和模式，代码会更加冗长。
