...
impl<'a> CreateIndexBuilder<'a> {
    ...
    // Extract common index building logic
    async fn build_index_metadata(&mut self) -> Result<IndexMetadata> {
    ...
    pub async fn execute_uncommitted(mut self) -> Result<IndexMetadata> {
        self.build_index_metadata().await
    }

    #[instrument(skip_all)]
    async fn execute(mut self) -> Result<()> {
        let index_metadata = self.build_index_metadata().await?;
        let transaction = Transaction::new(
            index_metadata.dataset_version,
            Operation::CreateIndex {
                new_indices: vec![index_metadata],
                removed_indices: vec![],
            },
            /*blobs_op= */ None,
            None,
