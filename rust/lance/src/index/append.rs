...
mod tests {
    ...
    async fn test_append_legacy_greek_fts_preserves_canonical_details() {
        ...
        use lance_index::scalar::inverted::{InvertedIndexParams, Language, METADATA_FILE};
        use lance_index::scalar::{FullTextSearchQuery, IndexStore};
        use prost::Message;
        ...
        let current_params = InvertedIndexParams::new("raw".to_string(), Language::Greek)
            .lance_tokenizer("text".to_string())
        ...
        // Model a released segment whose physical metadata predates the Greek
        // stemmer field. Preserve the JSON byte length so the committed file
        // size remains valid after rewriting the metadata file.
        let committed = dataset.load_indices_by_name(INDEX_NAME).await.unwrap();
        let legacy_store = LanceIndexStore::from_dataset_for_existing(&dataset, &committed[0])
            .await
            .unwrap();
        let metadata_reader = legacy_store.open_index_file(METADATA_FILE).await.unwrap();
        let mut physical_metadata = metadata_reader.schema().metadata.clone();
        let explicit_params = physical_metadata
            .get("params")
            .expect("physical metadata should contain params");
        assert!(explicit_params.contains("greek_stemmer"));
        let mut absent_params = serde_json::to_string(&legacy_params).unwrap();
        assert!(!absent_params.contains("greek_stemmer"));
        absent_params.extend(std::iter::repeat_n(
            ' ',
            explicit_params.len() - absent_params.len(),
        ));
        physical_metadata.insert("params".to_string(), absent_params);

        let metadata_batch = metadata_reader.read_range(0..1, None).await.unwrap();
        let metadata_schema = Arc::new(Schema::new(metadata_batch.schema().fields().clone()));
        let metadata_batch =
            RecordBatch::try_new(metadata_schema.clone(), metadata_batch.columns().to_vec())
                .unwrap();
        drop(metadata_reader);
        let mut metadata_writer = legacy_store
            .new_index_file(METADATA_FILE, metadata_schema)
            .await
            .unwrap();
        metadata_writer
            .write_record_batch(metadata_batch)
            .await
            .unwrap();
        metadata_writer
            .finish_with_metadata(physical_metadata)
            .await
            .unwrap();
        ...
        let first_params = crate::index::scalar::load_segment_params(&dataset, &segments[0])
            .await
            .unwrap();
        let second_params = crate::index::scalar::load_segment_params(&dataset, &segments[1])
            .await
            .unwrap();
