...
impl PhysicalExpr for RowAddrExpr {
    ...
    fn evaluate(&self, batch: &RecordBatch) -> Result<ColumnarValue> {
        let rowids = batch.column(self.rowid_pos)
            .as_primitive_opt::<UInt64Type>()
            .ok_or_else(|| {
                DataFusionError::Internal(
                    "RowAddrExpr: rowid column is not a UInt64Type".into(),
                )
            })?;
...
mod test {
    ...
    fn sample_row_id_index() -> Arc<RowIdIndex> {
        ...
        Arc::new(RowIdIndex::new(&[
            (0, Arc::new(row_id_seq)),
        ]).unwrap())
    }
    ...
    async fn test_row_ids_no_nulls() {
        ...
        // Verify the result
        assert_eq!(result.as_ref(), Arc::new(UInt64Array::from(vec![10, 20, 30])).as_ref() as &dyn Array);
    }
    ...
    async fn test_row_ids_with_nulls() {
        ...
        // Verify the result
        assert_eq!(result.as_ref(), Arc::new(UInt64Array::from(vec![Some(10), None, Some(30)])).as_ref() as &dyn Array);
    }
