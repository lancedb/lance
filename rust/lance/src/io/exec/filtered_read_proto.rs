...
fn fr_options_to_proto(
    ...
    let refine_filter_substrait = match &options.refine_filter {
        Some(expr) => Some(encode_substrait(
            crate::dataset::field_assignment::unbind_field_assignment_expression(
                dataset,
                expr.clone(),
            )?,
            filter_schema.clone(),
    ...
    if has_filters {
        ...
        if let Some(bytes) = &proto.refine_filter_substrait {
            ...
            options.refine_filter = Some(
                crate::dataset::field_assignment::bind_field_assignment_expression(
                    dataset,
                    expression,
                    options
                        .fragments
...
async fn plan_from_proto(
    ...
    if !proto.fragment_filter_ids.is_empty() {
        ...
        for bytes in &proto.filter_expressions {
            ...
            let expr = crate::dataset::field_assignment::bind_field_assignment_expression(
                dataset, expr, None,
            )
            .await?;
...
mod tests {
    ...
    use crate::dataset::{FieldAssignment, FieldAssignmentAlteration, UpdateBuilder};
    ...
    #[tokio::test]
    async fn test_assignment_filter_roundtrip_executes_after_snapshot_rebinding() {
        ...
        // Assignment transport uses the stable field ID, so it remains
        // serializable when the tracked field type is pruned from Substrait.
        let filter_schema = Arc::new(ArrowSchema::empty());

        let public_expression = is_assigned(datafusion_expr::col("x"));
        let bound_expression = crate::dataset::field_assignment::bind_field_assignment_expression(
            &dataset,
            public_expression,
            None,
        )
