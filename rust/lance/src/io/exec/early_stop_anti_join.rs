...
use datafusion::physical_plan::{
    ...
    PlanProperties, Statistics, execution_plan::{Boundedness, EmissionType},
};
...
impl ExecutionPlan for EarlyStopAntiJoinExec {
    ...
    fn execute(
        ...
        debug!(
            "Executing EarlyStopAntiJoinExec: partition={}, limit={:?}",
            partition, self.limit
        );

        // Step 1: Create future to build exclusion hash from exclusion child
        let exclusion_child = self.exclusion_child.clone();
        let context_clone = context.clone();
        let exclusion_future = async move {
            build_exclusion_hash(exclusion_child, partition, context_clone).await
        }
        .boxed();
    ...
    fn statistics(&self) -> DFResult<Statistics> {
        ...
        if let Some(limit) = self.limit {
            ...
            if let Some(num_rows) = scan_stats.num_rows.get_value() {
                ...
                stats = stats.with_num_rows(datafusion::common::stats::Precision::Inexact(estimated));
            }
            Ok(stats)
        }
    }
...
async fn build_exclusion_hash(
    ...
    info!("Built exclusion hash set with {} items", exclusion_set.len());
...
impl EarlyStopAntiJoinStream {
    ...
    fn filter_batch(&mut self, batch: RecordBatch) -> DFResult<Option<RecordBatch>> {
        ...
        if let Some(0) = self.remaining {
            debug!("Early stop: limit reached");
            return Ok(None);
        }
            ...
            if !exclusion_set.contains(&value) {
                ...
                if let Some(limit) = &mut self.remaining {
                    *limit = limit.saturating_sub(1);
                    if *limit == 0 {
                        debug!("Early stop: reached limit after {} kept rows", keep_indices.len());
                        break;
                    }
                }
            } else {
...
impl Stream for EarlyStopAntiJoinStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Option<Self::Item>> {
        ...
        if self.exclusion_set.is_none() {
            if let Some(mut future) = self.exclusion_future.take() {
                match future.poll_unpin(cx) {
                    Poll::Ready(Ok(set)) => {
                        debug!("Exclusion set built with {} items", set.len());
        ...
        if let Some(0) = self.remaining {
            info!(
                "Early stop complete: filtered {} rows, limit reached",
                self.total_filtered
            );
        ...
        match self.scan_stream.poll_next_unpin(cx) {
            ...
            Poll::Ready(None) => {
                info!(
                    "Stream complete: filtered {} rows total",
                    self.total_filtered
                );
                Poll::Ready(None)
            }
...
mod tests {
    ...
    async fn test_empty_exclusion_set() {
        ...
        let exclusion_child = Arc::new(MockExec::new(empty_batch));
