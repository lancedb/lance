...
/// Validate the identity used to prove ownership after an ambiguous commit.
///
/// Transaction UUIDs are normally generated by
/// [`crate::dataset::transaction::TransactionBuilder`]. Public bindings can
/// also construct transactions directly, so enforce the invariant at the
/// common commit boundary before writing any artifacts.
fn validate_transaction_identity(transaction: &Transaction) -> Result<()> {
    if transaction.uuid.is_empty() {
        return Err(Error::invalid_input(
            "Transaction UUID must not be empty because it identifies the commit attempt",
        ));
    }
    Ok(())
}

/// Compare both the stable attempt identity and its canonical persisted intent.
///
/// The protobuf representation is the transaction's persistence contract. It
/// normalizes fields such as index timestamps, so comparing it avoids false
/// negatives after a round trip while preventing a reused UUID from claiming a
...
/// the stable identity and canonical persisted intent recorded in the manifest
/// at `version` with this attempt's transaction.
///
            ...
            Ok(Some((manifest, location))) => {
                match read_manifest_transaction(object_store, base_path, &manifest, &location).await
                {
                    Ok(Some(committed_transaction)) => {
                        return if is_same_commit_attempt(&committed_transaction, transaction) {
                            CommitOutcome::Ours {
...
async fn do_commit_new_dataset(
    ...
    validate_transaction_identity(transaction)?;
...
mod tests {
    ...
    #[test]
    fn test_commit_attempt_match_requires_identity_and_intent() {
        use crate::dataset::transaction::TransactionBuilder;

        let attempted = TransactionBuilder::new(7, Operation::Append { fragments: vec![] })
            .uuid("shared-attempt-id".to_string())
            .tag(Some("attempted".to_string()))
            .build();
        let persisted = pb::Transaction::from(&attempted).try_into().unwrap();
        assert!(is_same_commit_attempt(&persisted, &attempted));

        let different_intent = TransactionBuilder::new(
            7,
            Operation::Delete {
        ...
        .uuid(attempted.uuid.clone())
        .tag(attempted.tag.clone())
        .build();
        assert!(
            !is_same_commit_attempt(&different_intent, &attempted),
            "a reused UUID must not claim a different transaction"
        );

        let missing_identity = TransactionBuilder::new(7, Operation::Append { fragments: vec![] })
            .uuid(String::new())
            .build();
        ...
        assert!(validate_transaction_identity(&missing_identity).is_err());
    }
    ...
    /// CreateIndex metadata is normalized by its protobuf round trip, so
    /// ownership must compare the attempt identity and canonical persisted
    /// intent instead of the decoded domain structs.
    #[tokio::test]
    async fn test_create_index_succeeds_when_conflict_is_own_commit() {
        use crate::utils::test::{AmbiguousCommitHandler, AmbiguousFailure};
        ...
        dataset
            .create_index(
                &["x"],
                IndexType::BTree,
                Some("x_idx".to_string()),
                &ScalarIndexParams::default(),
                true,
            )
            .await
            .expect("a landed CreateIndex commit must be recognized as our attempt");

        let fresh = Dataset::open(uri).await.unwrap();
        assert_eq!(fresh.version().version, 2);
        let indices = fresh.load_indices().await.unwrap();
        assert_eq!(indices.len(), 1);
        assert_eq!(indices[0].name, "x_idx");
    }
