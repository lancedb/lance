...
#[tokio::test]
async fn test_mismatch_config_writers() {
    // If some writers are using RenameCommitHandler and we are using DynamoDBExternalManifestStore,
    // we should fail in safe ways. (Never overwriting, always returning errors.)
    let datagen = gen().col("values", array::step::<Int32Type>());
    let data = datagen.into_batch_rows(RowCount::from(100)).unwrap();

    let bucket = S3Bucket::new("test-misconfigured").await;
    let ddb_table = DynamoDBCommitTable::new("test-misconfigured").await;
    let base_write_params = WriteParams {
        store_params: Some(ObjectStoreParams {
            storage_options: Some(
                CONFIG
                    .iter()
                    .filter(|(k, _)| *k != "dynamodb_endpoint")
                    .map(|(k, v)| (k.to_string(), v.to_string()))
                    .collect(),
            ),
            ..Default::default()
    ...
    let mut dataset = InsertBuilder::new(&format!("s3://{}/base/", bucket.0))
        .with_params(&base_write_params)
        .execute(vec![data.clone()])
        .await
        .unwrap();

    let mut dataset_ddb = DatasetBuilder::from_uri(format!(
        "s3+ddb://{}/base/?ddbTableName={}",
        bucket.0, ddb_table.0
    ))
    ...
    let mut both_succeeded = false;
    let mut base_succeeded = false;
    let mut ddb_succeeded = false;

    while attempts_left > 0 && (!both_succeeded || !base_succeeded || !ddb_succeeded) {
        let builder_base = InsertBuilder::new(Arc::new(dataset.clone()));
        let task_write_base = builder_base.execute(vec![data.clone()]);
        let builder_ddb = InsertBuilder::new(Arc::new(dataset_ddb.clone()));
        let task_write_ddb = builder_ddb.execute(vec![data.clone()]);
        let (res_base, res_ddb) = tokio::join!(task_write_base, task_write_ddb);

        let old_version = dataset.manifest().version;
        match (res_base, res_ddb) {
            (Ok(base_ds), Ok(ddb_ds)) => {
                dataset = base_ds;
                dataset_ddb = ddb_ds;
                both_succeeded = true;

                // If we checkout the DDB one first, that should resolve things for
                // the S3 one.
                dataset_ddb.checkout_latest().await.unwrap();
                dataset.checkout_latest().await.unwrap();

                assert_eq!(old_version + 2, dataset.manifest().version);
                assert_eq!(old_version + 2, dataset_ddb.manifest().version);
            }
            (Ok(base_ds), Err(_)) => {
                dataset = base_ds;
                base_succeeded = true;
                ...
                assert_eq!(old_version + 1, current_version);

                dataset_ddb.checkout_latest().await.unwrap();
                let current_version = dataset_ddb.manifest().version;
                assert_eq!(old_version + 1, current_version);
            }
            (Err(_), Ok(ddb_ds)) => {
                dataset_ddb = ddb_ds;
