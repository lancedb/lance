...
use lance_datafusion::udf::{AssignmentFragment, bound_is_assigned_snapshot};
...
impl IndexInformationProvider for ScalarIndexInfo {
    ...
    fn exact_row_selection(&self, expr: &Expr) -> Option<Arc<ExactRowSelection>> {
        ...
        let (field_id, fragments, covered_fragments) =
            bound_is_assigned_snapshot(function.func.as_ref())?;
        if fragments
            .keys()
        ...
        for (fragment_id, state) in fragments.iter() {
            match state {
                AssignmentFragment::All => rows.insert_fragment(*fragment_id),
                AssignmentFragment::Partial(bitmap) => {
                    rows.insert_bitmap(*fragment_id, bitmap.as_ref().clone());
                }
            }
        }
        Some(Arc::new(ExactRowSelection::new(
            format!("FieldAssignment(field_id={field_id})"),
            expr.clone(),
