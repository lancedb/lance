...
impl Dataset {
    ...
    pub async fn delete_branch(&mut self, branch: &str) -> Result<()> {
        ...
        if let Err(e) = self.object_store.delete(branch_location.base_path()).await {
            match &e {
                Error::IO { source, .. } => {
...
mod tests {
    ...
    async fn test_shallow_clone_multiple_times(
        ...
        for path in clone_paths.iter() {
            ...
            current_dataset
                .tags
                .create("v1", current_dataset.latest_version_id().await.unwrap(), None)
                .await
                .unwrap();
    ...
    async fn test_branch() {
        ...
        branch2_dataset
            .tags
            .create("tag1", branch2_dataset.version().version, Some("branch2"))
            .await
            .unwrap();

        let mut branch3_dataset = branch2_dataset
            .create_branch("branch3", "tag1", None)
            .await
            .unwrap();
        ...
        assert!(branches.contains_key("branch2"));
        assert!(branches.contains_key("branch3"));
        ...
        let branch2_content = branches.get("branch2").unwrap();
        ...
        let branch3_content = branches.get("branch3").unwrap();
        // Created based on tag pointed to branch2
        assert_eq!(branch3_content.parent_branch.as_deref().unwrap(), "branch2");
        ...
        // Drop datasets for releasing handles
        drop(branch1_dataset);
        drop(branch2_dataset);
        drop(branch3_dataset);
        drop(updated_branch1);
        drop(updated_branch2);
        drop(updated_branch3);
        drop(dataset);

        let mut dataset = main_dataset;
        // Finally delete all branches
        dataset.delete_branch("branch1").await.unwrap();
        dataset.delete_branch("branch2").await.unwrap();
        dataset.delete_branch("branch3").await.unwrap();
        ...
        let branch1_path = test_dir.path().join("branch1");
        let branch2_path = test_dir.path().join("branch2");
        let branch3_path = test_dir.path().join("branch3");
