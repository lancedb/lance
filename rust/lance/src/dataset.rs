...
mod tests {
    ...
    async fn test_geo_types() {
        ...
        let rect = Rect::new(
            coord! { x: -72.123, y: 42.1546 },
            coord! { x: -72.1260, y: 42.45 },
        );
        ...
        let reader = RecordBatchIterator::new(vec![batch].into_iter().map(Ok), schema.clone());
        let dataset = Dataset::write(reader, uri, Some(Default::default()))
            .await
            .unwrap();
        ...
        assert_eq!(
            fields.get(0).unwrap().metadata.get("ARROW:extension:name"),
