...
/// Cache key for an immutable field-assignment root or bitmap.
///
...
/// assignment objects from more than one external dataset base.
#[derive(Debug)]
pub struct FieldAssignmentFileKey<'a> {
    pub source_uri: &'a str,
    pub path: &'a str,
    pub size_bytes: u64,
}

impl CacheKey for FieldAssignmentFileKey<'_> {
    ...
    fn type_name() -> &'static str {
        "FieldAssignmentBytes"
    }

    fn schema() -> CacheKeySchema {
        CacheKeySchema::new("lance.dataset.field-assignment-file-key", 1)
    }
