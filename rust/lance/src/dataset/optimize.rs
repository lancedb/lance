...
async fn recalc_versions_for_rewritten_fragments(
    ...
    let mut old_version_sequences: Vec<lance_table::format::RowLatestUpdateVersionSequence> =
        Vec::with_capacity(old_fragments.len());
    for frag in old_fragments.iter() {
        let mut seq = if let Some(version_meta) = &frag.row_latest_update_version_meta {
            version_meta.load_sequence().map_err(|e| Error::Internal {
                message: format!("Failed to load RowLatestUpdateVersionSequence: {}", e),
                location: location!(),
            })?
        } else {
        ...
        .zip(new_version_sequences.into_iter())
    {
        fragment.row_latest_update_version_meta =
            Some(lance_table::format::RowLatestUpdateVersionMeta::from_sequence(&seq).unwrap());
    }
