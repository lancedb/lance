...
pub struct Scanner {
    ...
    /// Dataset-bound ordering expressions. Present only when cell flag
    /// binding rewrites an ordering key such as `cell_flag(field, name)`.
...
impl Scanner {
    ...
    /// expressions such as `cell_flag(embedding, 'computed')`.
    ///
    ...
    fn references_cell_flag(&self) -> Result<bool> {
        ...
        if let Some(filter) = &self.filter.expr_filter {
            let references_cell_flag = match filter {
                ExprFilter::Sql(sql) => sql.to_ascii_lowercase().contains("cell_flag"),
                ExprFilter::Substrait(_) => true,
                ExprFilter::Datafusion(expression) => expression_references_cell_flag(expression)?,
            };
...
mod test {
    ...
    #[tokio::test]
    async fn test_cell_flag_binding_fast_path_for_ordinary_scan() {
