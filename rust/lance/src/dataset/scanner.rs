...
"
  ...
  FilterExec: _distance@2 IS NOT NULL
    ...
    async fn test_limit_pushdown_with_index() -> Result<()> {
        ...
        // Execute and collect actual results
        // Note: We need to create the plan again since it was consumed by analyze
        let plan2 = scanner.create_plan().await?;
        let stream = plan2.execute(0, Arc::new(TaskContext::default()))?;
        let results = collect(stream).await?;
    ...
    async fn test_limit_pushdown_rejected_for_multiple_partitions() -> Result<()> {
        ...
        // Force multiple partitions by using scan_in_order(false) with specific settings
        let mut scanner = dataset.scan();
        scanner
            .limit(Some(10), None)?
            .batch_size(100)
            .batch_readahead(2)
            .scan_in_order(false) // This typically uses multiple partitions
            .project(&["i"])?;
