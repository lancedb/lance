    ...
    fn references_cell_flag(&self) -> Result<bool> {
        ...
        if let Some(filter) = &self.filter.expr_filter {
            let expression = match filter {
                ExprFilter::Datafusion(expression) => expression.clone(),
                ExprFilter::Sql(_) | ExprFilter::Substrait(_) => {
                    let filter_schema = self.filterable_schema()?;
                    filter.to_datafusion(self.dataset.schema(), filter_schema.as_ref())?
                }
            };
            let references_cell_flag = expression_references_cell_flag(&expression)?;
