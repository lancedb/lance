...
impl From<&pb::transaction::UpdateMap> for UpdateMap {
    ...
    fn modified_fragment_ids(&self) -> Box<dyn Iterator<Item = u64> + '_> {
        match self {
            ...
            | Self::UpdateConfig { .. }
            | Self::Restore { .. }
            | Self::UpdateMemWalState { .. } => Box::new(std::iter::empty()),
            Self::Delete {
                updated_fragments,
    ...
    /// Returns true if this operation modifies any of the same fragment IDs as another operation.
    fn modifies_same_ids(&self, other: &Self) -> bool {
        let self_ids: HashSet<u64> = self.modified_fragment_ids().collect();
        let other_ids: HashSet<u64> = other.modified_fragment_ids().collect();
        !self_ids.is_disjoint(&other_ids)
    }
}
    ...
    pub fn conflicts_with(&self, other: &Self) -> bool {
        ...
        match &self.operation {
            ...
            Operation::UpdateMemWalState { .. } => {
                // UpdateMemWalState doesn't conflict with any other operations
                // as it only modifies the MemWAL state tracking
                false
            }
        }
    }
