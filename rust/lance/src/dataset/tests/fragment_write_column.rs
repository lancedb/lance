...
/// A required child slot beneath a null nullable parent is not a value of the
/// child: Arrow masks it, and a scan of this same dataset produces exactly
/// this shape. Staging must accept it and round-trip the logical values.
/// (2.1+ only: the 2.0 logical encoders cannot store a masked child slot, so
/// that writer keeps rejecting it up front.)
#[rstest]
#[tokio::test]
async fn test_accepts_masked_null_under_required_child(
    #[values(LanceFileVersion::V2_1)] version: LanceFileVersion,
) {
    let point = ArrowField::new("point", DataType::Struct(point_children()), true);
    let arrow_schema = Arc::new(ArrowSchema::new(vec![point]));
    ...
    .unwrap();
    let dataset = dataset_of(seed, Some(version)).await;

    // [{x: 3, y: 30}, null] -- the hidden child slots are physically null.
    ...
    let schema = dataset.schema().clone();
    let group = stage(&dataset, staged, &schema).await.unwrap();
    let dataset = commit(&dataset, vec![group]).await.unwrap();

    let batch = dataset.scan().try_into_batch().await.unwrap();
    let column = batch.column(0).as_struct();
    assert!(column.is_valid(0), "row 0 keeps its value");
    assert!(column.is_null(1), "row 1 stays null");
    assert_eq!(column.column(0).as_primitive::<Int32Type>().value(0), 3);
    assert_eq!(column.column(1).as_primitive::<Int32Type>().value(0), 30);
}

/// Item nulls inside a null list slot's non-empty window are masked, not
/// values of the item field: the encoder never serializes a null slot's
/// window. Staging must accept them and round-trip the logical values.
#[tokio::test]
async fn test_accepts_masked_item_nulls_in_null_list_slot_window() {
    let item = |nullable| Arc::new(ArrowField::new("item", DataType::Int32, nullable));
    let list_batch = |values: ArrayRef, nulls, nullable| {
        let array = ListArray::new(
            item(nullable),
            OffsetBuffer::new(vec![0, 2, 4].into()),
            values,
        ...
        batch_of(
            vec![ArrowField::new("v", DataType::List(item(nullable)), true)],
            vec![Arc::new(array) as ArrayRef],
        )
    };
    let seed = list_batch(ints(vec![1, 2, 3, 4]), None, false);
    let dataset = dataset_of(seed, Some(LanceFileVersion::V2_1)).await;

    // Slot 1 is null; its window [2, 4) holds physical nulls. The manifest
    // item is non-null, but no reader can observe those slots.
    let staged = list_batch(
    ...
    assert!(list.is_valid(0), "slot 0 keeps its values");
    assert!(list.is_null(1), "slot 1 stays null");
    assert_eq!(
        list.value(0).as_primitive::<Int32Type>().values(),
        &[10, 20]
    );
}

/// A sliced list batch keeps child values outside its visible offset range;
/// a null there is unreachable and must not reject the batch.
#[tokio::test]
async fn test_accepts_sliced_list_with_unreachable_item_null() {
    let item = |nullable| Arc::new(ArrowField::new("item", DataType::Int32, nullable));
    let seed = batch_of(
        vec![ArrowField::new("v", DataType::List(item(false)), true)],
    ...
    // [[null, 20], [30, 40]] sliced to its second row: the only visible
    // values are [30, 40].
    let full = ListArray::new(
        item(true),
        OffsetBuffer::new(vec![0, 2, 4].into()),
        Arc::new(Int32Array::from(vec![None, Some(20), Some(30), Some(40)])) as ArrayRef,
        None,
    );
    let staged = batch_of(
        vec![ArrowField::new("v", DataType::List(item(true)), true)],
        vec![Arc::new(full.slice(1, 1)) as ArrayRef],
    );
