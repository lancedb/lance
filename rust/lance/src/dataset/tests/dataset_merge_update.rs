...
async fn test_data_replacement_populates_invalidated_bitmap() {
    ...
    assert!(idx.invalidated_fragment_bitmap.is_none());
    ...
    // Check: fragment 0 added to invalidated_fragment_bitmap
    assert!(
        idx.invalidated_fragment_bitmap
            .as_ref()
            .expect("invalidated_fragment_bitmap should be Some")
            .contains(0),
        "Fragment 0 should be in invalidated_fragment_bitmap"
    );
}
