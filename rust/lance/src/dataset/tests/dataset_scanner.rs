...
/// Test that when a user specifies a metric incompatible with the data type
/// (e.g., L2 on binary vectors), the index is still used with its own metric.
#[tokio::test]
async fn test_knn_incompatible_metric_uses_index() {
    use arrow_array::UInt8Array;

    // Create binary vectors (UInt8) - only Hamming is valid for these
    let vectors = UInt8Array::from(vec![0u8, 128, 255]);
    let vectors = FixedSizeListArray::try_new_from_values(vectors, 1).unwrap();
    let ids = Int32Array::from(vec![0, 1, 2]);
    let schema = Arc::new(ArrowSchema::new(vec![
        ArrowField::new(
            "vector",
            DataType::FixedSizeList(Arc::new(ArrowField::new("item", DataType::UInt8, true)), 1),
            false,
        ),
        ArrowField::new("id", DataType::Int32, false),
    ]));
    let batch =
        RecordBatch::try_new(schema.clone(), vec![Arc::new(vectors), Arc::new(ids)]).unwrap();
    let reader = RecordBatchIterator::new(vec![Ok(batch)], schema);

    let mut ds = Dataset::write(reader, "memory://test_incompatible_metric", None)
        .await
        .unwrap();

    // Create Hamming index (only valid metric for binary vectors)
    let params = VectorIndexParams::ivf_flat(1, MetricType::Hamming);
    ds.create_index(&["vector"], IndexType::Vector, None, &params, true)
        .await
        .unwrap();

    // Query with L2 metric (invalid for binary vectors) - should still work by using index
    let query_vec = UInt8Array::from(vec![128u8]);
    let query_vec = FixedSizeListArray::try_new_from_values(query_vec, 1).unwrap();

    let results = ds
        .scan()
        .nearest("vector", query_vec.values().as_ref(), 3)
        .unwrap()
        .distance_metric(MetricType::L2) // Invalid for binary, should be ignored
        .try_into_batch()
        .await
        .unwrap();

    // Should succeed and return results (using Hamming from the index)
    assert_eq!(results.num_rows(), 3);
    assert!(results.column_by_name(DIST_COL).is_some());
}
