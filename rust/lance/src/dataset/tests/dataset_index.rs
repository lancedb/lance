...
async fn test_legacy_json_index_without_target_type_does_not_block_compaction() {
    let test_dir = copy_test_data_to_tmp("v10.0.0/legacy_json_compaction").unwrap();
    let mut dataset = Dataset::open(&test_dir.path_str()).await.unwrap();

    let indices = dataset.load_indices().await.unwrap();
    assert_eq!(indices.len(), 2);
    assert!(indices.iter().all(|index| index.index_version == 0));
    assert_eq!(
        indices
            .iter()
            .map(|index| index.name.as_str())
            .collect::<HashSet<_>>(),
        HashSet::from(["json_bitmap", "json_ngram"])
    );
    ...
    crate::dataset::optimize::compact_files(
        &mut dataset,
        crate::dataset::optimize::CompactionOptions::default(),
        None,
    )
    .await
    .unwrap();
