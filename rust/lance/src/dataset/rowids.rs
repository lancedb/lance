...
mod test {
    ...
    use crate::{dataset::{builder::DatasetBuilder, UpdateBuilder, WriteMode, WriteParams}, index::scalar::ScalarIndexParams};
    ...
    async fn test_delete_with_row_ids(
        #[values(true, false)] with_scalar_index: bool,
    ) {
        let batch = sequence_batch(0..6);
        ...
        if with_scalar_index {
            dataset
                .create_index(&["id"], IndexType::Scalar, None, &ScalarIndexParams::default(), false)
                .await
                .unwrap();
        }
