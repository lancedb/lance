...
pub trait RefOperations<T> {
    ...
    /// Update a tag or branch, branch is not supported to update now
    async fn update(
        &mut self,
        name: &str,
        version_number: u64,
...
impl RefOperations<TagContents> for Tags {
    ...
    async fn update(&mut self, tag: &str, version: u64, branch: Option<&str>) -> Result<()> {
        check_valid_ref(tag)?;
        ...
        let manifest_file = self
            .refs
            .commit_handler
            .resolve_version_location(&base_path, version, &self.refs.object_store.inner)
            .await?;
...
impl RefOperations<BranchContents> for Branches {
    ...
    async fn update(
        ...
        Err(Error::not_supported(
            format!(
                "update_branch to version_number {} of {:?}",
