...
pub(super) async fn indexed_vector_dataset_with_metric(
    ...
    // Two partitions so the plan exercises the multi-partition path, and 8 sub-vectors so PQ's
    // quantization error stays small enough for the recall assertions to mean something.
    let params = VectorIndexParams::ivf_pq(2, 8, 8, metric, 50);
    dataset
