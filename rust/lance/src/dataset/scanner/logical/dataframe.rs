...
fn with_take(
    ...
    // A row's version columns come from its position within its fragment, so only an ordered scan
    // can produce them and a take never can. A frame with no projection of its own asks for every
    // column the leaf advertises, these among them, so they are dropped here rather than left for
    // the read to be asked for something it cannot return.
    projection.with_row_last_updated_at_version = false;
    projection.with_row_created_at_version = false;
