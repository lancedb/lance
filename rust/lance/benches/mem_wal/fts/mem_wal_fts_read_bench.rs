...
async fn run_prepare(args: &Args) -> Result<()> {
    ...
    let mut dataset = Dataset::write(reader, &args.uri, Some(WriteParams { data_storage_version: Some(lance_file::version::LanceFileVersion::V2_2), ..Default::default() })).await?;
    println!(
...
async fn build_and_query_baseline(
    ...
    let mut merged = Dataset::write(reader, &merged_uri, Some(WriteParams { data_storage_version: Some(lance_file::version::LanceFileVersion::V2_2), ..Default::default() })).await?;
    merged
        .create_index(
            &[TEXT_COL],
            IndexType::Inverted,
