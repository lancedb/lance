...
async fn build_reference_dataset(
    ...
    let reader = RecordBatchIterator::new(batches.into_iter(), schema.clone());
    let mut dataset = Dataset::write(reader, ref_uri, Some(WriteParams { data_storage_version: Some(lance_file::version::LanceFileVersion::V2_2), ..Default::default() })).await?;
    dataset
        .create_index(
            &[TEXT_COL],
            IndexType::Inverted,
