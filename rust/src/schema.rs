...
impl Field {
    ...
    pub fn type_str(t: &DataType) -> String {
        match t {
            ...
            DataType::Time32(unit) => format!("time32:{}", to_str(unit)).as_str(),
            DataType::Time64(unit) => format!("time64:{}", to_str(unit)).as_str(),
            DataType::Timestamp(unit, _) => format!("timestamp:{}", to_str(unit)).as_str(),
            DataType::Binary => "binary",
            DataType::Utf8 => "string",
            DataType::LargeBinary => "largebinary",
            DataType::LargeUtf8 => "largestring",
            DataType::FixedSizeBinary(len) => format!("fixed_size_binary:{}", len).as_str(),
            DataType::FixedSizeList(v, len) => format!("fixed_size_list:{}:{}", Self::type_str(v.data_type()), len).as_str(),
            _ => panic!(),
        }.to_string()
    }
