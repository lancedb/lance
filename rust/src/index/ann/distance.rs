...
#[inline]
fn euclidean_distance_native(from: &Float32Array, to: &Float32Array) -> f32 {
    let sub = subtract(to, from).unwrap();
    let mul = multiply(&sub, &sub).unwrap();
    sum(&mul).unwrap_or(0.0)
}
...
pub fn euclidean_distance(
    ...
    assert_eq!(to.value_length() % 8, 0);

    let inner_array = to.values();
    let buffer = inner_array.data_ref().buffer::<f32>(0);
        ...
        Float32Array::from_trusted_len_iter(
            (0..to.len())
                .map(|idx| {
                    ...
                    euclidean_distance_native(from, arr)
                })
                .map(|d| Some(d)),
        )
    };
...
mod tests {
    ...
    fn test_euclidean_distance() {
        ...
        // let native_socres = euclidean_distance(from, to)
    }
}
