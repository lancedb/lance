...
//! Core plugin traits for scalar index implementations.
...
use crate::scalar::{
    CreatedIndex, IndexStore, RowIdRemapper, ScalarIndex, TrainingCriteria,
    expression::ScalarQueryParser,
};
...
pub trait ScalarIndexPlugin: Send + Sync + std::fmt::Debug {
    ...
    ///
    /// Training an index can be a complex process. This method is a compromise:
    /// indexes that support training on a stream of column data should override
    /// this to return `Some(self)`. Indexes that need their own individualized
    /// training approaches should return `None` and provide their own methods.
    ...
    /// representation under a sized [`CacheKey`] with a codec.
    ...
    ///
    /// Called after all plugins are registered. Plugins that need to delegate to
    /// other plugins (e.g. [`JsonIndexPlugin`]) should store the registry reference here.
    fn attach_registry(&self, _registry: Arc<dyn crate::registry::IndexRegistry>) {}
...
/// is cached under `state_key`. Every caller — warm hits included — then rebuilds
/// the index from the shared state via `from_state`, an IO-free reconstruct.
...
/// per-index namespaced by the caller, so a constant key suffices.
