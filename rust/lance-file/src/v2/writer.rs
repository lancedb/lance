...
use lance_encoding::encoder::fill_in_buffer_locations;
...
impl FileWriter {
    ...
    async fn write_page(&mut self, mut encoded_page: EncodedPage) -> Result<()> {
        ...
        let buffer_locs = buffer_offsets
            .iter()
            .copied()
            .zip(buffer_sizes.iter().copied());
        fill_in_buffer_locations(&mut encoded_page.encoding, buffer_locs);
        let page = pbfile::column_metadata::Page {
            buffer_offsets,
            buffer_sizes,
            encoding: Some(pbfile::Encoding {
                style: Some(pbfile::encoding::Style::Direct(DirectEncoding {
                    encoding: Some(Any::from_msg(&encoded_page.encoding)?),
                })),
            }),
    ...
    async fn write_pages(
        &mut self,
        mut encoding_tasks: FuturesUnordered<BoxFuture<'static, Result<Vec<EncodedPage>>>>,
    ) -> Result<()> {
        ...
        while let Some(encoding_task) = encoding_tasks.next().await {
            let encoded_pages = encoding_task?;
    ...
    pub async fn finish(&mut self) -> Result<()> {
        ...
        let encoding_tasks = self
            .column_writers
            .iter_mut()
            .filter_map(|writer| writer.flush())
