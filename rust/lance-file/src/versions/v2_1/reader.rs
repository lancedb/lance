...
fn required<'a, T>(value: Option<&'a T>, label: &str) -> Result<&'a T> {
    value.ok_or_else(|| {
        Error::invalid_input_source(
            format!("Lance v2.1 {label} is missing its nested encoding").into(),
        )
    })
}
        ...
        Some(Compression::Rle(rle)) => {
            ...
            if !fixed_values || !fixed_u8_lengths {
                return Err(Error::invalid_input_source(
                    "Lance v2.1 RLE requires flat values and flat u8 run lengths".into(),
                ));
            }
            Ok(())
        }
    ...
    match layout.layout.as_ref() {
        Some(Layout::MiniBlockLayout(miniblock)) => {
            if miniblock.has_large_chunk {
                return Err(Error::invalid_input_source(
                    "Large miniblock chunks are not part of the Lance v2.1 grammar".into(),
                ));
            }
        ...
        Some(Layout::ConstantLayout(constant)) => {
            if constant.inline_value.is_some() {
                Err(Error::invalid_input_source(
                    "Lance v2.1 only accepts the all-null form of constant page layout".into(),
                ))
            } else if constant.rep_compression.is_some() || constant.def_compression.is_some() {
                Err(Error::invalid_input_source(
                    "Compressed constant-page levels are not part of the Lance v2.1 grammar".into(),
                ))
            } else {
                Ok(())
            }
        ...
        Some(Layout::BlobLayout(blob)) => {
            let inner = blob.inner_layout.as_deref().ok_or_else(|| {
                Error::invalid_input_source(
                    "Lance v2.1 blob page layout is missing its inner layout".into(),
                )
            })?;
            validate_page_layout(inner)
        }
        Some(Layout::SparseLayout(_)) => Err(Error::invalid_input_source(
            "Sparse page layout is not part of the Lance v2.1 grammar".into(),
        )),
        None => Err(Error::invalid_input_source(
            "Lance v2.1 page is missing its page layout".into(),
        )),
    }
}
...
pub fn finish_metadata(raw: RawFileMetadata) -> Result<CachedFileMetadata> {
    if (raw.footer.major_version, raw.footer.minor_version) != (2, 1) {
        return Err(Error::version_conflict(
            "Attempt to use the Lance v2.1 reader for a different file version".to_string(),
    ...
    Ok(CachedFileMetadata {
        ...
        version: ConcreteFileVersion::V2_1,
...
pub fn finish_metadata_index(index: FileMetadataIndex) -> Result<FileMetadataIndex> {
    if index.version != ConcreteFileVersion::V2_1 {
        let (major, minor) = index.version.to_standard_footer_numbers();
        return Err(Error::version_conflict(
            "Attempt to use the Lance v2.1 reader for a different metadata index".to_string(),
            major,
            minor,
        ));
    }
...
pub fn test_projection_length(
    ...
    structural::test_projection_length(schema, column_indices, column_lengths)
}
...
mod grammar_tests {
    use super::*;
    use pb21::{
        CompressiveEncoding, FullZipLayout, General, PageLayout, VariablePackedStruct,
        compressive_encoding::Compression, page_layout::Layout,
    };

    #[test]
    fn rejects_nested_variable_packed_struct() {
        let variable_packed = CompressiveEncoding {
            compression: Some(Compression::VariablePackedStruct(VariablePackedStruct {
                fields: Vec::new(),
            })),
        };
        let wrapped = CompressiveEncoding {
            compression: Some(Compression::General(Box::new(General {
                compression: None,
                values: Some(Box::new(variable_packed)),
            }))),
        };
        let layout = PageLayout {
            layout: Some(Layout::FullZipLayout(FullZipLayout {
                value_compression: Some(wrapped),
                ..Default::default()
            })),
        };
