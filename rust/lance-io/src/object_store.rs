...
pub use read_dir::{
    DirCursor, DirEntry, DirEntryKind, DirPage, PaginatedDirLister, ReadDirOptions,
};
...
pub trait WrappingObjectStore: std::fmt::Debug + Send + Sync {
    ...
    ///
    /// [`ObjectStore::read_dir_stream`] pushes the resume position into the backend's own
...
impl ObjectStore {
    ...
    pub async fn from_uri_and_params(
        ...
        if let Some((store, path)) = params.object_store.as_ref() {
            ...
            let store = Self {
                ...
                io_tracker,
                store_prefix,
                // The store was handed to us already type-erased, so there is no way to
                // tell whether it can paginate.
    ...
    /// Use [`Self::read_dir_stream`] to page through a directory instead.
...
impl ObjectStore {
    ...
    pub fn new(
        ...
        Self {
            ...
            io_tracker,
            store_prefix,
            // The store was handed to us already type-erased, so there is no way to tell
            // whether it can paginate.
            paginated_lister: None,
        }
    }
}
