...
//! way of asking — [`native`] over `object_store`'s own paginated API and [`opendal`] over
//! OpenDAL's `Lister`.
...
impl ObjectStore {
    ...
    ///
    /// On backends with a paginated list API — S3, GCS and Azure, plus OpenDAL-backed stores
    /// whose service can resume from a key — the resume position and the page size are pushed
    /// into the list request, so the page costs what the page holds. Elsewhere the directory
    /// is listed in full and paged locally, which is correct but no cheaper than
    /// [`Self::read_dir`].
    ///
