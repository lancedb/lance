...
/// A wrapper around an object store that tracks IO stats.
///
/// Note: this doesn't work with the default local filesystem, which has specialized
/// methods that bypass the object store interface. Either use the `memory://` or
/// `file-object-store://` schemes instead with this.
