    ...
    async fn new_store(
        ...
        let (inner, paginated_lister) = if use_opendal {
            ...
            let store = self
                ...
                .await?;
            // S3 Express ignores `start-after`, so a paginated listing there would restart
            // from the beginning of the prefix on every page.
            let paginated_lister =
                (!is_s3_express).then(|| NativeDirLister::for_store(store.clone()));
