...
impl Conformance {
    ...
    async fn names(store: &ObjectStore, dir: &str, options: ReadDirOptions) -> Vec<String> {
        store
            .read_dir_stream(Path::from(dir), options)
            .map_ok(|entry| label(&entry))
            .try_collect()
            .await
            .unwrap()
    }
    ...
    async fn reference_names(&self, dir: &str) -> Vec<String> {
        let names = Self::names(&self.reference, dir, ReadDirOptions::default()).await;
    ...
    /// Paging must be invisible: whatever the page size, and whether the stream paginates
    /// internally or the caller resumes from a cursor, the result is the reference listing.
    async fn assert_paging_is_invisible(&self, dir: &str) {
        ...
        for &page_size in PAGE_SIZES {
            ...
            let streamed = Self::names(&self.paginated, dir, options).await;
            assert_eq!(
                streamed, reference,
                "page_size {page_size} changed the listing"
            );

            assert_eq!(
                self.walk_by_cursor(dir, page_size).await,
                reference,
                "resuming from a cursor with page_size {page_size} changed the listing"
            );
        }
    }

    /// Page through the directory the way a caller would: take a page, ask for the next one
    /// starting from the last entry's cursor.
    async fn walk_by_cursor(&self, dir: &str, page_size: usize) -> Vec<String> {
        ...
        loop {
            ...
            seen.extend(page.iter().map(label));
            assert!(
                seen.len() <= SHAPES.len() + MARKED_CHILDREN.len(),
                "the walk is serving entries more than once: {seen:?}"
            );
        }
    }
}
...
async fn test_a_listing_without_a_page_size_is_complete(
    ...
    let listed = Conformance::names(&stores.paginated, DIR, ReadDirOptions::default()).await;
...
/// What a page boundary landing on a directory costs. The listing is correct either way; the
/// question is how many requests it takes, which is what the real-cloud run measures.
#[rstest]
#[tokio::test]
async fn test_resuming_at_a_directory_costs_one_page_per_child(
    ...
    // A store that spends its limit per entry answers in one request per child: asking for
    // one entry more than the page needs means the last request finds the end of the
    // directory rather than costing another. A store that spends its limit on the keys it
    // scans cannot, since resuming at `a.lance/` re-scans the twenty keys that prefix
    // collapses, so the page has to widen first — which is the cost the real-cloud
    // measurement below pins down.
    let requests = emulator.requests();
    match budget {
        BudgetMode::PerEntry => assert_eq!(requests, 3, "expected one request per child"),
        BudgetMode::PerScannedKey => assert!(
            (4..=12).contains(&requests),
            "widening should recover in a handful of requests, took {requests}"
        ),
    }
}
...
mod cloud {
    ...
    async fn test_page_cost_against_a_real_store() {
        ...
        for page_size in [1usize, 2, 10] {
            ...
            // One request per child plus one to find the end is the floor. Anything far
            // above it means the store spends its page limit on the keys it scans, so
            // resuming inside `big.lance/` needs the page to widen first.
            assert!(
                requests <= 32,
