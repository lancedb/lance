...
//! Benchmark IVF partition assignment across a range of partition counts.
//!
...
fn bench_partition_computer(c: &mut Criterion) {
    ...
    let batch = make_input_batch(BATCH_SIZE, DIMENSION);
    ...
    let partition_counts: Vec<usize> = [1_000u64, 10_000, 100_000, 1_000_000, 10_000_000, 100_000_000, 1_000_000_000]
        .iter()
        .map(|&n| (n as f64).sqrt() as usize)
        .collect();

    for &num_partitions in &partition_counts {
        let centroids_flat = generate_random_array_with_seed::<Float32Type>(
            num_partitions * DIMENSION,
            SEED,
        );
        let centroids =
            FixedSizeListArray::try_new_from_values(centroids_flat, DIMENSION as i32).unwrap();
        ...
        group.bench_with_input(
            BenchmarkId::new("L2", format!("k={}", num_partitions)),
            &num_partitions,
            |b, _| {
                b.iter(|| {
