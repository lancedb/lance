...
async fn create_geo_index(points: Vec<(f64, f64, u64)>) -> (Arc<GeoIndex>, Arc<LanceIndexStore>, TempObjDir) {
...
fn bench_geo_index_intersects_pruning(c: &mut Criterion) {
    ...
    for num_points in [10_000, 100_000, 1_000_000] {
                ...
                b.iter(|| {
                    ...
                    rt.block_on(async {
                        for query_bbox in queries.iter() {
                            let _result = index.search_all_leaves(*query_bbox, counter.as_ref()).await.unwrap();
                        }
                    });
                });
            },
        ...
        group.bench_with_input(
            ...
            |b, (index, queries, counter)| {
                ...
                b.iter(|| {
                    ...
                    rt.block_on(async {
                        for query_bbox in queries.iter() {
                            let query = GeoQuery::Intersects(query_bbox[0], query_bbox[1], query_bbox[2], query_bbox[3]);
        ...
        for query_bbox in queries.iter() {
            rt.block_on(async {
                let _ = index.search_all_leaves(*query_bbox, &NoOpMetricsCollector).await;
            });
        }
...
criterion_group!(benches, bench_geo_index_intersects_pruning, bench_geo_intersects_query_size);
