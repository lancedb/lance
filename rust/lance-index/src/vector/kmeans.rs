...
use arrow_array::{Array, ArrayRef, FixedSizeListArray, PrimitiveArray};
...
/// Train KMeans model and returns the centroids of each cluster.
#[allow(clippy::too_many_arguments)]
pub async fn train_kmeans_v2<T: ArrowPrimitiveType>(
    ...
    let data = if num_rows > sample_rate * k {
        ...
        let mut builder = Vec::with_capacity(sample_size * dimension);
        for idx in chosen.iter() {
            let s = &array.values()[idx * dimension..(idx + 1) * dimension];
            builder.extend_from_slice(s);
        }
        PrimitiveArray::<T>::from_iter_values(builder)
    } else {
        array.clone()
    };
