...
mod tests {
    ...
    use crate::vector::distributed::ivf_coordinator::{DistributedIvfCoordinator, Fragment};
    ...
    async fn test_kmeans_allreduce_matches_single_worker() {
        ...
        let mid = all.len() / 2;
        let fsl_rank0 = make_fsl(all[..mid].to_vec());
        let fsl_rank1 = make_fsl(all[mid..].to_vec());
    ...
    async fn test_distributed_ivf_flat_build_matches_single_worker_assignments() {
        ...
        let merged = make_fsl(all.clone());
        let dim = merged.value_length() as usize;

        // Baseline: single-machine clustering + assignment
        let mut coord = DistributedIvfCoordinator::new(IvfBuildParams::new(k), 1);
        let centroids = coord
            .train_kmeans_distributed(merged.clone(), k)
            .await
            .expect("train failed");
        ...
        let mut merged_counts = lengths.iter().map(|x| *x as usize).collect::<Vec<_>>();
