...
fn calculate_centroid_similarity(
    ...
    Ok(avg_similarity.min(1.0).max(-1.0))
}
...
pub async fn merge_partition_data(
    ...
    log::info!("Partition {} merge completed: {} vectors", partition_id, merged_storage.len());
...
impl SupportedIndexType {
    ...
    fn detect(
        ...
        let is_pq = reader.metadata().file_schema.metadata.contains_key(PQ_METADATA_KEY) || has_pq_code_col;
        let is_sq = reader.metadata().file_schema.metadata.contains_key(SQ_METADATA_KEY) || has_sq_code_col;
        
        // Detect HNSW-related columns
        let has_hnsw_vector_id_col = schema
            .fields
            .iter()
            .any(|f| f.name() == "__vector_id");
        let has_hnsw_pointer_col = schema
            .fields
            .iter()
            .any(|f| f.name() == "__pointer");
        let has_hnsw = has_hnsw_vector_id_col || has_hnsw_pointer_col;
        
        let index_type = match (has_hnsw, is_pq, is_sq) {
            (false, false, false) => SupportedIndexType::IvfFlat,
            (false, true, false) => SupportedIndexType::IvfPq,
            (false, false, true) => SupportedIndexType::IvfSq,
            (true, false, false) => SupportedIndexType::IvfHnswFlat,
            (true, true, false) => SupportedIndexType::IvfHnswPq,
            (true, false, true) => SupportedIndexType::IvfHnswSq,
            _ => {
    ...
    fn as_str(&self) -> &'static str {
        match self {
            SupportedIndexType::IvfFlat => "IVF_FLAT",
            SupportedIndexType::IvfPq => "IVF_PQ",
            SupportedIndexType::IvfSq => "IVF_SQ",
            SupportedIndexType::IvfHnswFlat => "IVF_HNSW_FLAT",
            SupportedIndexType::IvfHnswPq => "IVF_HNSW_PQ",
            SupportedIndexType::IvfHnswSq => "IVF_HNSW_SQ",
        }
    }
}
...
pub async fn merge_vector_index_files(
    ...
    for aux in &aux_paths {
        ...
        let ivf_idx: u32 = reader.metadata()
            .file_schema
            .metadata
        ...
        match detected_index_type.unwrap() {
            SupportedIndexType::IvfSq => {
                ...
                let sq_json = if let Some(sq_json) = reader.metadata().file_schema.metadata.get(SQ_METADATA_KEY) {
                    sq_json.clone()
                } else if let Some(storage_meta_json) =
                    reader.metadata().file_schema.metadata.get(STORAGE_METADATA_KEY)
                {
                ...
                let sq_meta_parsed: ScalarQuantizationMetadata =
                    serde_json::from_str(&sq_json).map_err(|e| Error::Index {
            ...
            SupportedIndexType::IvfPq => {
                ...
                let pm_json = if let Some(pm_json) = reader.metadata().file_schema.metadata.get(PQ_METADATA_KEY) {
                    pm_json.clone()
                } else if let Some(storage_meta_json) =
                    reader.metadata().file_schema.metadata.get(STORAGE_METADATA_KEY)
                {
                ...
                if let Some(existing_pm) = pq_meta.as_ref() {
                    ...
                    let a = if let Some(cb) = existing_pm.codebook.as_ref() {
                        cb.values().as_primitive::<arrow::datatypes::Float32Type>().values().to_vec()
                    } else {
                        return Err(Error::Index {
            ...
            SupportedIndexType::IvfHnswFlat | SupportedIndexType::IvfHnswPq | SupportedIndexType::IvfHnswSq => {
        ...
        for pid in 0..nlist {
            ...
            while let Some(rb) = stream.next().await {
                ...
                if let Some(w) = v2w_opt.as_mut() {
                    ...
                    if !matches!(detected_index_type.unwrap(), SupportedIndexType::IvfHnswFlat | SupportedIndexType::IvfHnswPq | SupportedIndexType::IvfHnswSq) {
            ...
            for aux in &aux_paths {
                ...
                while let Some(rb) = stream.next().await {
                    ...
                    if let Some(col) = rb.column_by_name(ROW_ID_FIELD.name()) {
                        ...
                        for i in 0..arr.len() {
                            let v = arr.value(i);
                            minv = Some(match minv { Some(m) => m.min(v), None => v });
                            maxv = Some(match maxv { Some(m) => m.max(v), None => v });
                        }
                    } else {
                        return Err(Error::Index {
    ...
    if let Some(w) = v2w_opt.as_mut() {
        ...
        let dt2 = distance_type.ok_or_else(|| Error::Index { message: "Distance type missing".to_string(), location: location!() })?;
