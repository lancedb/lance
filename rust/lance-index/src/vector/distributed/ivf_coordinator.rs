    ...
    pub async fn sample_vectors(
        ...
        if let Some(fsl) = self.sample_override.as_ref() {
            ...
            let values = fsl
                ...
                .values();
            let slice =
                Float32Array::from(values[..(sample_size as usize * dim as usize)].to_vec());
...
impl DistributedIvfCoordinator {
    ...
    async fn train_distributed_ivf_with_retry(
        ...
        let fragment_samples = stream::iter(fragments)
            .map(|fragment| async move {
                ...
                log::debug!("Fragment {}: {} vectors from {} total", fragment.id, local_sample_size, fragment_size);
    ...
    fn merge_fragment_samples(
        ...
        log::info!("Merging {} samples, {} vectors, dim {}", fragment_samples.len(), total_samples, dim);
    ...
    pub async fn train_kmeans_allreduce(
        ...
        if local_data.len() + 0 < num_partitions && comm.rank() == 0 {
            return Err(Error::invalid_input(
                format!(
                    "k {} greater than local sample {}; training may be unstable",
                    num_partitions, local_data.len()
                ),
                location!(),
            ));
        }
