...
/// Structural roles of one leaf in a compound plan.
///
/// The Boolean and Boost flags record ancestry rather than claiming that an
/// individual leaf is globally required. For example, both alternatives of a
/// `MultiMatch` below a MUST edge have `in_must`, while only the selected
/// generator cover has `generator`.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub(super) struct CompoundLeafRoles {
    pub(super) generator: bool,
    pub(super) in_must: bool,
    pub(super) in_should: bool,
    pub(super) in_must_not: bool,
    pub(super) in_negative_boost: bool,
}
...
pub(super) struct CompoundPlanAnalysis {
    ...
    pub(super) leaf_roles: Vec<CompoundLeafRoles>,
}
...
struct LeafRoleContext {
    in_must: bool,
    in_should: bool,
    in_must_not: bool,
    in_negative_boost: bool,
}
...
impl CompoundScorerPlan {
    ...
    pub(super) fn analyze_leaves(
        ...
        let mut leaf_roles = vec![CompoundLeafRoles::default(); leaf_count];
        self.collect_leaf_roles(LeafRoleContext::default(), &mut leaf_roles)?;
        for &leaf in &node.generator_leaves {
            leaf_roles[leaf].generator = true;
        }
    ...
    fn collect_leaf_roles(
        &self,
        context: LeafRoleContext,
        roles: &mut [CompoundLeafRoles],
    ) -> Result<()> {
        match self {
            Self::Leaf { index, .. } => {
                let role = roles.get_mut(*index).ok_or_else(|| {
                    Error::internal(format!(
                        "compound FTS plan references missing leaf role {index}"
                    ))
                })?;
                role.in_must |= context.in_must;
                role.in_should |= context.in_should;
                role.in_must_not |= context.in_must_not;
                role.in_negative_boost |= context.in_negative_boost;
                Ok(())
            }
            Self::Boost {
                positive, negative, ..
            } => {
                positive.collect_leaf_roles(context, roles)?;
                negative.collect_leaf_roles(
                    LeafRoleContext {
                        in_negative_boost: true,
                        ..context
                    },
                    roles,
                )
            ...
            Self::MultiMatch(children) => {
                for child in children {
                    child.collect_leaf_roles(context, roles)?;
                }
                Ok(())
            }
            Self::Boolean {
...
mod tests {
    ...
    #[test]
    fn staged_plan_analysis_selects_stable_must_generator_and_roles() {
        ...
        assert!(analysis.leaf_roles[0].in_should);
        assert!(!analysis.leaf_roles[0].generator);
        for index in [1, 2, 3, 4] {
            assert!(analysis.leaf_roles[index].in_must);
        }
        assert!(analysis.leaf_roles[1].generator);
        assert!(analysis.leaf_roles[2].generator);
        assert!(!analysis.leaf_roles[3].generator);
        assert!(analysis.leaf_roles[4].in_negative_boost);
        assert!(analysis.leaf_roles[5].in_must_not);
    }
    ...
    fn staged_plan_analysis_handles_optional_missing_and_impossible_required() {
        ...
        assert!(!analysis.possible);
        assert!(analysis.generator_leaves.is_empty());
        assert!(analysis.leaf_roles[0].in_must_not);
    }
    ...
    fn staged_plan_analysis_composes_signed_nested_boost_and_unbounded_inputs() {
        ...
        assert!(!analysis.leaf_roles[0].in_negative_boost);
        assert!(analysis.leaf_roles[1].in_negative_boost);
        assert!(analysis.leaf_roles[2].in_negative_boost);
