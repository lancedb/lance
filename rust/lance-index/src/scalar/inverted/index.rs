...
impl InvertedIndex {
    ...
    pub async fn bm25_search_documents(
        ...
        struct MergedCandidate {
            score: OrderedFloat,
            /// Index into the per-query partition list for deferred document-key
            /// resolution.
            slot: u32,
            addr: CandidateAddr,
            /// The partition-local document id for modern posting lists.
            posting_doc_id: u64,
        }
        ...
        // Resolve logical document keys only for candidates that survived the
        // global top-k. Row documents only defer `Pending` addresses, while
        ...
        for (pos, Reverse(candidate)) in sorted.into_iter().enumerate() {
            if coordinate_rank > 0 {
                let doc_id = u32::try_from(candidate.posting_doc_id).map_err(|_| {
                ...
                deferred
                    ...
                    .or_default()
                    .push((pos, doc_id));
                documents.push(ScoredDoc::with_doc_index(0, Vec::new(), candidate.score.0));
                continue;
            }

            match candidate.addr {
                CandidateAddr::RowId(row_id) => {
                    documents.push(ScoredDoc::new(row_id, candidate.score.0));
                }
                CandidateAddr::Pending(doc_id) => {
                    deferred
                        ...
                        .or_default()
                        .push((pos, doc_id));
                    documents.push(ScoredDoc::new(0, candidate.score.0));
                }
            }
        }
        if !deferred.is_empty() {
            ...
            let resolved: Vec<ResolvedGroup> =
                stream::iter(groups.into_iter().map(|(part, entries)| async move {
                    ...
                    if resolved.len() != entries.len() {
                        return Err(Error::internal(
                            "FTS document-key resolution returned an unexpected number of items"
                                .to_string(),
                        ));
                    }
                ...
                .try_collect()
                .await?;
            for (entries, resolved_keys) in resolved {
                for ((pos, _), (row_id, doc_index)) in entries.into_iter().zip(resolved_keys) {
                    documents[pos].row_id = row_id;
                    documents[pos].doc_index = doc_index;
                }
            }
        }
        Ok(documents)
