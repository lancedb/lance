...
impl PartitionDocuments {
    ...
    pub(crate) async fn resolve_addresses(&self, doc_ids: &[DocId]) -> Result<Vec<u64>> {
        ...
        if let Some(row_ids) = self.shared_addresses.load().upgrade() {
            return Ok(doc_ids
                .iter()
        ...
        if self.prefer_sparse_document_read(doc_ids.len()) {
            ...
            if row_ids.null_count() != 0 || row_ids.len() != doc_ids.len() {
                return Err(corrupt_docs(
                    &self.path,
                    format!(
                        "sparse {ROW_ID} projection returned {} rows with {} nulls for {} candidates",
                        row_ids.len(),
                        row_ids.null_count(),
                        doc_ids.len()
                    ),
            ...
            return Ok(row_ids.values().to_vec());
        }
    ...
    pub(crate) fn estimated_address_read_bytes(&self, doc_ids: &[DocId]) -> usize {
        ...
        if self.remapper.is_none() && self.prefer_sparse_document_read(doc_ids.len()) {
            return doc_ids.len().saturating_mul(std::mem::size_of::<u64>());
        }
...
mod tests {
    ...
    async fn final_address_resolution_reuses_the_cached_row_id_column() {
        ...
        assert_eq!(
            ...
            point_ids.len() * std::mem::size_of::<u64>()
        );
        assert_eq!(
        ...
        assert_eq!(counts.address_rows.load(Ordering::Relaxed), point_ids.len());
        assert!(
            cache
                .get_with_key(&DocRowIdsKey { partition_id: 0 })
                .await
        ...
        assert_eq!(counts.range_calls.load(Ordering::Relaxed), 1);
        assert_eq!(
            counts.address_rows.load(Ordering::Relaxed),
            point_ids.len() + num_docs as usize
        );
        assert!(
            cache
                .get_with_key(&DocRowIdsKey { partition_id: 0 })
