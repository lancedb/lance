...
// Note: u128 has no entry — its `BitPacking` impl is a single scalar loop and does not
// generate per-width specialized functions, so `seq_t!` is never invoked for u128.
macro_rules! seq_t {
...
// u128 bit-packing: scalar bit-stream implementation packing 1024 u128 values at a given
// runtime-known width. Unlike u8/u16/u32/u64 which have per-width transposed kernels
// generated by macros, u128 uses a single sequential loop. This is intentional:
//
//   * Generating 128 width-specialized functions would bloat compile time substantially.
//   * Rust has no native 128-bit SIMD lowering; a transposed kernel would still emit
//     scalar IR on most targets.
//
...
impl BitPacking for u128 {
    ...
    unsafe fn unchecked_pack(width: usize, input: &[Self], output: &mut [Self]) {
        ...
        for &val in input.iter() {
            let val = val & mask;
            let word_idx = bit_offset / Self::T;
            let bit_idx = bit_offset % Self::T;
            output[word_idx] |= val << bit_idx;
            // Carry into the next word only when `val` straddles the word boundary.
            // When this branch is taken, `bit_idx + width > Self::T` plus the
            // `width < Self::T` invariant from above forces `bit_idx > 0`, so
            // `Self::T - bit_idx` lies in `1..Self::T` and the right-shift is defined
            // (`>> Self::T` on u128 would be UB).
            if bit_idx + width > Self::T {
                output[word_idx + 1] |= val >> (Self::T - bit_idx);
            }
            bit_offset += width;
        }
    }
    ...
    unsafe fn unchecked_unpack(width: usize, input: &[Self], output: &mut [Self]) {
        ...
        // Past the early returns: `1 <= width < Self::T`, so `1 << width` and the
        // `Self::T - bit_idx` shift in the carry path are well-defined (the carry only
        // runs when `bit_idx + width > Self::T`, which combined with `width < Self::T`
        // forces `bit_idx > 0`).
        debug_assert!(width < Self::T);
        let mask: Self = (1 << width) - 1;
        let mut bit_offset: usize = 0;
...
mod test {
    ...
    fn unchecked_pack_unpack_u128(bit_width: usize) {
        let mut values = [0u128; 1024];
        let mut rng = XorShift::new(987654321);
        // Fill values bounded by `1 << bit_width` so the truth set covers the entire range
        // [0, 2^bit_width), not just zeros. Skip the random fill at width 0 (all zeros by
        // definition); at the maximum width use the full 128-bit range with no mask.
        if bit_width == u128::T {
            for value in &mut values {
        ...
        let mut packed = vec![0u128; 1024 * bit_width / u128::T];
        unsafe {
            BitPacking::unchecked_pack(bit_width, &values, &mut packed);
        }
        let mut output = [0u128; 1024];
        unsafe { BitPacking::unchecked_unpack(bit_width, &packed, &mut output) };
        assert_eq!(values, output);
    }
    ...
    #[test]
    fn test_unchecked_pack_u128() {
        // Boundary widths: 0 (all-zero), 1 (smallest carry pattern), 7/8/24 (typical
        // decimal128 widths after sign-extension), 63/64/65 (u64 boundary on either
        // side), 96/127 (high-bit shifts), 128 (full width — no compression, fast
        // copy path).
        for &width in &[0usize, 1, 7, 8, 24, 63, 64, 65, 96, 127, 128] {
            unchecked_pack_unpack_u128(width);
        }

        // Sanity: a deterministic input ramps through pack/unpack at width=24, the
        // typical packed width for Decimal128(7,2).
        let input: [u128; 1024] = array::from_fn(|i| (i as u128) & ((1u128 << 24) - 1));
        let mut packed = vec![0u128; 1024 * 24 / u128::T];
        unsafe { BitPacking::unchecked_pack(24, &input, &mut packed) };
        let mut output = [0u128; 1024];
        unsafe { BitPacking::unchecked_unpack(24, &packed, &mut output) };
        assert_eq!(input, output);
    }
}
