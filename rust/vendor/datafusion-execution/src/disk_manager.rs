...
pub struct DiskManager {
    ...
    /// Default to 100GB
    max_temp_directory_size: u64,
    /// Serializes file usage changes so limit failures do not mutate global usage.
...
mod tests {
    ...
    #[test]
    fn test_failed_disk_usage_update_does_not_change_usage() -> Result<()> {
        let dm = Arc::new(
            DiskManagerBuilder::default()
                .with_max_temp_directory_size(1)
                .build()?,
        );
        let mut first = dm.create_tmp_file("Testing accepted disk usage")?;
        first.inner().as_file().set_len(1)?;
        first.update_disk_usage()?;

        let mut rejected = dm.create_tmp_file("Testing rejected disk usage")?;
        rejected.inner().as_file().set_len(1)?;
        assert!(matches!(
