...
impl<'a> RepDefStructSchedulingJob<'a> {
    fn new(
        scheduler: &'a RepDefStructScheduler,
...
impl<'a> StructuralSchedulingJob for RepDefStructSchedulingJob<'a> {
    fn schedule_next(
        ...
        while old_rows_scheduled == self.rows_scheduled {
            ...
            // Once a child is done we can assume the other children are done
...
/// record batch as a non-nullable struct.
#[derive(Debug)]
pub struct RepDefStructScheduler {
    children: Vec<Box<dyn StructuralFieldScheduler>>,
    child_fields: Fields,
}

impl RepDefStructScheduler {
...
impl StructuralFieldScheduler for RepDefStructScheduler {
    fn schedule_ranges<'a>(
        &'a self,
        ranges: &[Range<u64>],
        filter: &FilterExpression,
...
impl StructuralStructDecoder {
    ...
    fn field_to_decoder(
        ...
        match field.data_type() {
            DataType::Struct(fields) => Box::new(StructuralStructDecoder::new(
                fields.clone(),
                should_validate,
            )),
...
pub struct RepDefStructFieldEncoder {
    children: Vec<Box<dyn FieldEncoder>>,
}

impl RepDefStructFieldEncoder {
    pub fn new(children: Vec<Box<dyn FieldEncoder>>) -> Self {
        Self { children }
    }
}

impl FieldEncoder for RepDefStructFieldEncoder {
    fn maybe_encode(
        &mut self,
        array: ArrayRef,
        ...
        } else if struct_array.null_count() == struct_array.len() {
            repdef.add_all_null(struct_array.len());
        } else {
            repdef.add_validity_bitmap(struct_array.nulls().cloned().unwrap());
        }
        let child_tasks = self
            .children
            .iter_mut()
