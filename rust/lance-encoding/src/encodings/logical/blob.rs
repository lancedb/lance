...
use crate::{
    ...
    encoder::{
        EncodeTask, EncodedColumn, FieldEncoder, OutOfLineBuffers,
    },
...
impl BlobStructuralEncoder {
    pub fn new(
        ...
        let mut descriptor_metadata = HashMap::new();
        ...
        let descriptor_field = Field::try_from(
            ArrowField::new("", descriptor_data_type, false)
        ...
        for i in 0..binary_array.len() {
            if binary_array.is_null(i) {
                ...
                if value.is_empty() {
                    ...
                    let position = external_buffers.add_buffer(
                        LanceBuffer::Borrowed(value.into())
                    );
...
mod tests {
    ...
    use crate::{
        encoder::{EncodingOptions, ColumnIndexSequence},
    ...
    fn test_blob_encoder_creation() {
        let field = Field::try_from(
            ArrowField::new("blob_field", DataType::LargeBinary, true)
        ).unwrap();
    ...
    async fn test_blob_encoding_simple() {
        let field = Field::try_from(
            ArrowField::new("blob_field", DataType::LargeBinary, true)
                .with_metadata(HashMap::from([
                    (lance_core::datatypes::BLOB_META_KEY.to_string(), "true".to_string()),
                ]))
        ).unwrap();
        let mut column_index = ColumnIndexSequence::default();
        let column_idx = column_index.next_column_index(0);
        ...
        let mut tasks = encoder.maybe_encode(
            array.clone(),
        ...
        if tasks.is_empty() {
            tasks = encoder.flush(&mut external_buffers).unwrap();
        }
