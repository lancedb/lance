...
impl FixedSizeListBlock {
    pub fn num_values(&self) -> Result<u64> {
        ...
        child_values.checked_div(self.dimension)
                    .ok_or(Error::InvalidInput {
                     source: format!("Division by zero: dimension cannot be zero").into(), location: location!(),
                    })
    }
...
impl DictionaryDataBlock {
    fn decode_helper<K: ArrowDictionaryKeyType>(self) -> Result<DataBlock> {
        ...
        let estimated_size_bytes = {
            ...
            self.dictionary.data_size()
            * (self.indices.num_values + dict_num_values - 1)
            / dict_num_values
        };
