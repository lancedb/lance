...
fn arrow_dictionary_to_data_block(
    ...
    let items_field =
        Field::new("", values.data_type().clone(), true).with_metadata(field.metadata().clone());

    let items = DataBlock::from_array(values, &items_field);
...
impl DataBlock {
    ...
    pub fn from_arrays(arrays: &[ArrayRef], num_values: u64, field: &Field) -> Self {
        let data_type = field.data_type();

        debug_assert!(arrays
            .iter()
            .all(|arr| arr.data_type() == field.data_type()));
        ...
        let encoded = match data_type {
            ...
            DataType::Struct(fields) => {
                ...
                for (child_idx, child_field) in fields.iter().enumerate() {
                    let child_vec = structs
                        ...
                        .collect::<Vec<_>>();
                    children.push(Self::from_arrays(&child_vec, num_values, child_field));
                }
            ...
            DataType::FixedSizeList(child_field, dim) => {
                let children = arrays
                    ...
                    .collect::<Vec<_>>();
                let child_block =
                    Self::from_arrays(&children, num_values * *dim as u64, &child_field);
    ...
    pub fn from_array<T: Array + 'static>(array: T, field: &Field) -> Self {
...
mod tests {
    ...
    fn test_string_to_data_block() {
        ...
        let block = DataBlock::from_arrays(arrays, 7, &field);
        ...
        let block = DataBlock::from_arrays(arrays, 3, &field);
    ...
    fn test_string_sliced() {
        ...
        let check = |arr: Vec<StringArray>, expected_off: Vec<i32>, expected_data: &[u8]| {
            ...
            let data = DataBlock::from_arrays(&arrs, num_rows, &field);
    ...
    fn test_large() {
        ...
        let data = DataBlock::from_array(arr, &field);
    ...
    fn test_dictionary_indices_normalized() {
        ...
        let data = DataBlock::from_arrays(&[Arc::new(arr1), Arc::new(arr2)], 5, &field);
    ...
    fn test_dictionary_nulls() {
        ...
        let data = DataBlock::from_array(dict, &field);
    ...
    fn test_dictionary_cannot_concatenate() {
        ...
        let data = DataBlock::from_arrays(&[Arc::new(dict1), Arc::new(dict2)], 512, &field);
    ...
    fn test_enum_dictionary() {
        ...
        let data = DataBlock::from_arrays(&[dict1.clone(), dict2.clone()], 1000, &field);
        let data = data.as_dictionary().unwrap();
        ...
        let data = DataBlock::from_arrays(&[dict1.clone(), dict2.clone()], 1000, &enum_field);
        let data = data.as_dictionary().unwrap();
    ...
    fn test_data_size() {
        ...
        let block = DataBlock::from_array(arr.clone(), &field);
        ...
        let block = DataBlock::from_arrays(&[arr1.clone(), arr2.clone(), arr3.clone()], 9, &field);
