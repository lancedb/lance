...
use crate::encodings::logical::r#struct::{
    RepDefStructScheduler, SimpleStructDecoder, SimpleStructScheduler, StructuralStructDecoder,
};
...
impl ColumnInfo {
    ...
    pub fn is_structural(&self) -> bool {
        self.page_infos
            .iter()
            .any(|page| page.encoding.is_structural())
    }
}
...
impl RootScheduler {
    ...
    fn as_structural(&self) -> &Box<dyn StructuralFieldScheduler> {
        match self {
            Self::Structural(s) => s,
...
impl CoreFieldDecoderStrategy {
    ...
    fn create_structural_field_scheduler<'a>(
        &self,
        field: &Field,
        column_infos: &mut ColumnInfoIter,
        buffers: FileBuffers,
        ...
        match &data_type {
            DataType::Struct(fields) => {
                ...
                for field in field.children.iter() {
                    let field_scheduler =
                        self.create_structural_field_scheduler(field, column_infos, buffers)?;
                    child_schedulers.push(field_scheduler);
                }

                let fields = fields.clone();
                Ok(
                    Box::new(RepDefStructScheduler::new(child_schedulers, fields))
    ...
    fn create_legacy_field_scheduler<'a>(
        &self,
        field: &Field,
        column_infos: &mut ColumnInfoIter,
        buffers: FileBuffers,
    ...
    pub async fn try_new<'a>(
        ...
        if column_infos[0].is_structural() {
            ...
            let mut root_scheduler = CoreFieldDecoderStrategy::default()
                .create_structural_field_scheduler(&root_field, &mut column_iter, buffers)?;
...
impl MessageType {
    pub fn into_legacy(self) -> DecoderReady {
        match self {
            MessageType::DecoderReady(decoder) => decoder,
            MessageType::UnloadedPage(_) => {
    ...
    pub fn into_structural(self) -> UnloadedPage {
        match self {
            MessageType::UnloadedPage(unloaded) => unloaded,
