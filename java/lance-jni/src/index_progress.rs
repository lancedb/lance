...
impl JavaIndexBuildProgress {
    ...
    fn call_stage_start(
        ...
        let stage_obj = env
            .new_string(stage)
            .map_err(|error| callback_error(&mut env, "stageStart", stage, error))?;
        let unit_obj = env
            .new_string(unit)
            .map_err(|error| callback_error(&mut env, "stageStart", stage, error))?;
    ...
    fn call_stage_progress(&self, stage: &str, completed: u64) -> lance_core::Result<()> {
        ...
        let stage_obj = env
            .new_string(stage)
            .map_err(|error| callback_error(&mut env, "stageProgress", stage, error))?;
    ...
    fn call_stage_complete(&self, stage: &str) -> lance_core::Result<()> {
        ...
        let stage_obj = env
            .new_string(stage)
            .map_err(|error| callback_error(&mut env, "stageComplete", stage, error))?;
...
fn callback_error(
    ...
    lance_core::Error::invalid_input(format!(
        "IndexBuildProgress.{method} callback failed for stage '{stage}': {error}"
    ))
}
