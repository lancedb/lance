...
public class CreateEmptyIndexTest {
  ...
  void testCreateEmptyBtreeIndex(@TempDir Path tempPath) throws Exception {
    ...
    List<Field> fields =
        Arrays.asList(
            Field.nullable(
                ...
                new FieldType(
                    true,
                    org.apache.arrow.vector.types.pojo.ArrowType.Int.createInt(32, true),
                    null)));
    ...
    try (IntVector idVector = new IntVector("id", allocator)) {
      ...
      try (Dataset dataset =
          Dataset.create(allocator, datasetPath, schema, vectors, Optional.empty())) {
    ...
    List<Field> fields =
        Arrays.asList(
            Field.nullable(
                "category",
                new FieldType(
                    true, org.apache.arrow.vector.types.pojo.ArrowType.Utf8.INSTANCE, null)));
    Schema schema = new Schema(fields);
