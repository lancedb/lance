 ...
 *
 * <p>These tests cover basic functionality, error handling, boundary conditions, and resource
 * management for the MergeInsertBuilder class.
 */
public class MergeInsertTest {
  ...
  void testMergeInsertResultWithLargeValues() {
    MergeInsertResult result =
        new MergeInsertResult(Long.MAX_VALUE, Long.MAX_VALUE, Long.MAX_VALUE);
  ...
  void testMergeInsertBuilderWithNullConditions() {
    ...
    MergeInsertBuilder configured =
        builder.whenMatchedUpdateAll(null).whenNotMatchedBySourceDelete(null);
  ...
  void testMergeInsertBuilderWithEmptyConditions() {
    ...
    MergeInsertBuilder configured =
        builder.whenMatchedUpdateAll("").whenNotMatchedBySourceDelete("");
  ...
  void testMergeInsertBuilderWithVeryLongConditions() {
    ...
    String longCondition =
        "source.value > target.value AND source.name != target.name AND source.id > 0 AND target.id > 0 AND source.value < 1000000 AND target.value < 1000000";
    MergeInsertBuilder configured =
        builder.whenMatchedUpdateAll(longCondition).whenNotMatchedBySourceDelete(longCondition);
  ...
  void testMergeInsertBuilderWithSpecialCharactersInConditions() {
    ...
    String specialCondition =
        "source.name = 'test\"quote' AND source.value > 0 AND target.name LIKE '%test%'";
    MergeInsertBuilder configured =
        builder
  ...
  void testMergeInsertBuilderWithAllColumns() {
    ...
    MergeInsertBuilder builder =
        MergeInsertBuilder.create(dataset, Arrays.asList("id", "name", "value"));
  ...
  void testMergeInsertBuilderWithClosedDataset() {
    ...
    Dataset closedDataset =
        Dataset.create(
            allocator,
            tempDir.resolve("closed_test").toString(),
            schema,
  ...
  void testMergeInsertBuilderPerformanceWithManyColumns() {
    ...
    Dataset manyColumnDataset =
        Dataset.create(allocator, datasetPath, manyColumnSchema, new WriteParams.Builder().build());
  ...
  void testMergeInsertBuilderWithDifferentDataTypes() {
    ...
    Dataset complexDataset =
        Dataset.create(allocator, datasetPath, complexSchema, new WriteParams.Builder().build());

    // Test with different column combinations
    MergeInsertBuilder builder1 = MergeInsertBuilder.create(complexDataset, "id");
    assertNotNull(builder1);

    MergeInsertBuilder builder2 =
        MergeInsertBuilder.create(complexDataset, Arrays.asList("id", "name"));
    assertNotNull(builder2);

    MergeInsertBuilder builder3 =
        MergeInsertBuilder.create(complexDataset, Arrays.asList("id", "name", "value", "flag"));
    assertNotNull(builder3);
