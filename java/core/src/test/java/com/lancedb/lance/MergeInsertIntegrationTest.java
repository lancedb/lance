 ...
 *
 * <p>These tests verify the complete merge insert workflow including data creation, merge insert
...
public class MergeInsertIntegrationTest {
  ...
  private VectorSchemaRoot createTestData(
      List<Integer> ids, List<String> names, List<Integer> values) {
  ...
  void testBasicMergeInsertInsertOnly() throws IOException {
    ...
    try (Dataset initialDataset =
        Dataset.create(
            allocator,
            tempDir.resolve("initial").toString(),
            schema,
    ...
    try (MergeInsertBuilder builder = MergeInsertBuilder.create(dataset, "id")) {
      ...
      MergeInsertBuilder configured = builder.whenNotMatchedInsertAll();
  ...
  void testMergeInsertWithUpdateConfiguration() {
    ...
    try (MergeInsertBuilder builder = MergeInsertBuilder.create(dataset, "id")) {
      MergeInsertBuilder configured = builder.whenMatchedUpdateAll().whenNotMatchedInsertAll();
  ...
  void testMergeInsertWithConditionalUpdate() {
    ...
    try (MergeInsertBuilder builder = MergeInsertBuilder.create(dataset, "id")) {
      MergeInsertBuilder configured =
          builder.whenMatchedUpdateAll("source.value > target.value").whenNotMatchedInsertAll();
  ...
  void testMergeInsertBuilderWithMultipleColumns() {
    ...
    try (MergeInsertBuilder builder =
        MergeInsertBuilder.create(dataset, Arrays.asList("id", "name"))) {
      MergeInsertBuilder configured = builder.whenMatchedUpdateAll().whenNotMatchedInsertAll();
